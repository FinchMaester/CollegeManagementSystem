    ...
    public function attendancereport()
{
    $this->session->set_userdata('top_menu', 'Reports');
    $this->session->set_userdata('sub_menu', 'Reports/attendance');
    $this->session->set_userdata('subsub_menu', 'Reports/attendencereport');
    ...
    // Resolve date range from search type
    ...
    // Build working/off days (kept for future use; not used below now)
    while ($current <= $last) {
    ...
    $data['attendance_type_id'] = $attendance_type_id = (int)$this->input->post('attendance_type');
    $filters = [
        'class_id'           => (int)$this->input->post('class_id', true),
        'section_id'         => (int)$this->input->post('section_id', true),
        'program_id'         => $this->input->post('program_id') ? (int)$this->input->post('program_id', true) : null, // optional
        'attendance_type_id' => $attendance_type_id,
        'date_from'          => $from_date,
        'date_to'            => $to_date,
    ];

    // Fetch rows (now only students matching selected type will appear)
    $data['student_attendences'] = $this->stuattendence_model->student_attendences($filters);

    // IMPORTANT: remove the old "all_week" loop that referenced $value['date']
    // (It caused "Undefined array key 'date'" because the new result is aggregated.)
    ...
    public function daily_attendance_report()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $date_ymd = $this->customlib->dateFormatToYYYYMMDD($this->input->post('date'));
            $date         = " and student_attendences.date='" . $date_ymd . "'";
