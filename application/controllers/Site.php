        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($result && $result->email != "") {
                if ($result->is_active == '1') {
                    ...
                    $sent = $this->mailsmsconf->mailsms('forgot_password', $sender_details);
                    if ($sent === true) {
                        $this->session->set_flashdata('message', $this->lang->line('please_check_your_email_to_recover_your_password'));
                        redirect('site/login', 'refresh');
                        return;
                    }
                    $data['error_message'] = $this->lang->line('email_send_failed_please_check_mail_settings_or_contact_administrator');
                } else {
    ...
    {        
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($result && $result->email != "") {
                ...
                $data['error_message'] = $this->lang->line('invalid_email_or_user_type');
            }
