...
class Financereports extends Admin_Controller
{
        ...
        if ($this->form_validation->run() == true) {
            ...
            $normalizeFeeDateToTimestamp = function ($rawDate) {
                ...
                if (preg_match('/^(\d{4})[-\/\.](\d{1,2})[-\/\.](\d{1,2})$/', $rawDate, $m)) {
                    $normalized = $m[1] . '-' . str_pad($m[2], 2, '0', STR_PAD_LEFT) . '-' . str_pad($m[3], 2, '0', STR_PAD_LEFT);
                    return strtotime($normalized);
                }

                // Legacy M-D-YY / M-D-YYYY values (seen in fee collection modal)
                if (preg_match('/^(\d{1,2})[-\/\.](\d{1,2})[-\/\.](\d{2,4})$/', $rawDate, $m)) {
                    $year = (int) $m[3];
                    if ($year < 100) {
                        // Fee collection BS dates used 2-digit years like 82 => 2082.
                        $year += 2000;
                    }
                    $normalized = $year . '-' . str_pad($m[1], 2, '0', STR_PAD_LEFT) . '-' . str_pad($m[2], 2, '0', STR_PAD_LEFT);
                    return strtotime($normalized);
                }

                // Last fallback for any uncommon string format.
                    ...
                    if (isJSON($fee_value->amount_detail)) {
                        ...
                        if (!empty($fees_details)) {
                            foreach ($fees_details as $fees_detail_key => $fees_detail_value) {
                                $date = $normalizeFeeDateToTimestamp(isset($fees_detail_value->date) ? $fees_detail_value->date : '');
                                if ($date !== false && $date >= $formated_date_from && $date <= $formated_date_to) {
    ...
    public function reportbyname()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == "GET") {
        ...
        } else {
            {
                ...
                // Fallback: show selected student profile even if no fee master rows exist yet.
                if (empty($student_due_fee) && !empty($student_id)) {
                    $student_list = $this->student_model->searchByClassSectionWithSession($class_id, $section_id, null, $program_id);
                    if (!empty($student_list)) {
                        foreach ($student_list as $student_row) {
                            if ((int) $student_row['id'] !== (int) $student_id) {
                                continue;
                            }
                            $student_session_id = isset($student_row['student_session_id']) ? $student_row['student_session_id'] : 0;
                            $student_due_fee[$student_session_id] = array(
                                'student_session_id' => $student_session_id,
                                'firstname' => isset($student_row['firstname']) ? $student_row['firstname'] : '',
                                'middlename' => isset($student_row['middlename']) ? $student_row['middlename'] : '',
                                'lastname' => isset($student_row['lastname']) ? $student_row['lastname'] : '',
                                'class_id' => isset($student_row['class_id']) ? $student_row['class_id'] : '',
                                'class' => isset($student_row['class']) ? $student_row['class'] : '',
                                'section' => isset($student_row['section']) ? $student_row['section'] : '',
                                ...
                                'mobileno' => isset($student_row['mobileno']) ? $student_row['mobileno'] : '',
                                'roll_no' => isset($student_row['roll_no']) ? $student_row['roll_no'] : '',
                                'category_id' => isset($student_row['category_id']) ? $student_row['category_id'] : '',
                                'category' => isset($student_row['category']) ? $student_row['category'] : '',
                                'rte' => isset($student_row['rte']) ? $student_row['rte'] : '',
    ...
    public function collection_report()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['results'] = $this->studentfeemaster_model->getFeeCollectionReport($start_date, $end_date, $feetype_id, $received_by, $group, $class_id, $section_id, $program_id);
    ...
    public function getincomelistbydt()
    {
        ...
        $start_date_ad = date('Y-m-d', strtotime($dates['from_date']));
        $end_date_ad   = date('Y-m-d', strtotime($dates['to_date']));


        $data['label'] = date($this->customlib->getSchoolDateFormat(), strtotime($start_date_ad)) . " " . $this->lang->line('to') . " " . date($this->customlib->getSchoolDateFormat(), strtotime($end_date_ad));

        // Convert AD dates to BS dates for database search (database stores BS dates)
        $start_date = $this->customlib->convertADDateToBS($start_date_ad);
        $end_date = $this->customlib->convertADDateToBS($end_date_ad);
