...
class Homework extends Admin_Controller
{
    ...
    public function searchdailyassignmentreport()
    {
        ...
        if (empty($class_id) || empty($section_id) || empty($subject_group_id) || empty($subject_group_subject_id)) {
            echo json_encode([
                "draw" => 0,
                "recordsTotal" => 0,
                "recordsFiltered" => 0,
                "data" => [],
                "error" => "Missing required parameters. Please select all criteria."
            ]);
            return;
        }
    ...
    public function dailyassignmentreportvalidation()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            // Return DataTables-compatible empty result with error message
            $json_data = array(
                "draw" => 0,
                "recordsTotal" => 0,
                "recordsFiltered" => 0,
