...
class Feediscount extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        foreach ($feesdiscount_result as $fk => $fd) {
            $feesdiscount_result[$fk]['applicable_summary'] = $this->feediscount_model->getApplicableFeetypeSummary($fd['id']);
        }
        $data['feediscountList'] = $feesdiscount_result;
        $data['feetype_list']    = $this->feediscount_model->getAllFeetypesForSelect();
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($insert_id) {
                $feetype_ids = $this->input->post('feetype_ids');
                if (!is_array($feetype_ids)) {
                    $feetype_ids = array();
                }
                $this->feediscount_model->saveApplicableFeetypes($insert_id, $feetype_ids);
            }
    ...
    public function edit($id)
    {
        ...
        $data['selected_feetype_ids'] = $this->feediscount_model->getApplicableFeetypeIds($id);
        $data['feetype_list']         = $this->feediscount_model->getAllFeetypesForSelect();
        $this->form_validation->set_rules('code', $this->lang->line('discount_code'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('name', $this->lang->line('name'), 'trim|required|xss_clean');
        if ($this->input->post('account_type') == "percentage") {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->feediscount_model->saveApplicableFeetypes($id, $feetype_ids);
    ...
    public function assign($id)
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            // Store POST values for form repopulation
