...
class Payroll extends Admin_Controller
{
    ...
    // Nepali Bikram Sambat months mapping
    ...
    // Get month dropdown in Nepali
    private function getNepaliMonthDropdown()
    {
        return $this->getNepaliMonths();
    }

    // Get available BS years
    private function getBSYears()
    {
        $years = [];
        $currentBSYear = 2082; // Current BS year - adjust this to your actual current BS year
        
        for ($i = $currentBSYear - 5; $i <= $currentBSYear + 1; $i++) {
            $years[$i] = $this->convertToNepaliDigits((string)$i) . ' (BS)';
        }
        return $years;
    }

    // Convert AD date to BS month name
    private function getBSMonthFromADDate($ad_date)
    {
        // Parse AD date
        $timestamp = strtotime($ad_date);
        $ad_month = (int)date('n', $timestamp); // 1-12
        $ad_year = (int)date('Y', $timestamp);
        
        // Simple AD to BS month conversion (approximate)
        // BS months start around mid-April (Baisakh)
        // This is a simplified conversion - for accurate conversion, use a proper library
        $bs_month = $ad_month;
        
        // Adjust for BS calendar (BS year starts in mid-April)
        if ($ad_month >= 4) {
            // April onwards maps to same BS month number
            $bs_month = $ad_month - 3; // April (4) = Baisakh (1), May (5) = Jestha (2), etc.
        } else {
            // Jan-Mar maps to previous BS year months
            $bs_month = $ad_month + 9; // Jan (1) = Magh (10), Feb (2) = Falgun (11), Mar (3) = Chaitra (12)
        }
        
        // Ensure month is between 1-12
        if ($bs_month < 1) $bs_month = 12;
        if ($bs_month > 12) $bs_month = $bs_month - 12;
        
        $nepali_months = $this->getNepaliMonths();
        return isset($nepali_months[$bs_month]) ? $nepali_months[$bs_month] : date("F", $timestamp);
    }
    ...
    public function index()
    {
        ...
        $data["staff_id"]            = "";
        $data["name"]                = "";
        $data["month"]               = date("n"); // numeric month (1-12)
        $data["year"]                = 2082; // BS year
        $data["present"]             = 0;
        $data["absent"]              = 0;
        $data["late"]                = 0;
        $data["half_day"]            = 0;
        ...
        $data['monthlist']           = $this->getNepaliMonthDropdown(); // Nepali months
        $data['yearlist']            = $this->getBSYears(); // BS years
        $data['sch_setting']         = $this->sch_setting_detail;
        $data['staffid_auto_insert'] = $this->sch_setting_detail->staffid_auto_insert;
        
        // IMPORTANT: Keep month as numeric value for URL generation
        ...
        if (isset($submit) && $submit == "search") {
            ...
            $data["month"]         = $month;
            $data["year"]          = $year;
            $data['month_numeric'] = $month; // Keep numeric for URL
            $data['year_numeric']  = $year;  // Keep numeric for URL
        }
    ...
    public function create($month, $year, $id, $role = '')
    {       
        ...
        $data["leave_count"]         = 0;
        $data["alloted_leave"]       = 0;
        $data["redirect_role"]       = $role; // Store role for redirect
        $data['sch_setting']         = $this->sch_setting_detail;
        $data['staffid_auto_insert'] = $this->sch_setting_detail->staffid_auto_insert;
        $user_type                   = $this->staff_model->getStaffRole();
        $data['classlist']           = $user_type;

        // Use BS date format (month and year are already BS, numeric)
        ...
        $data["nepali_months"]   = $this->getNepaliMonths(); // Pass Nepali months to view
        $data["redirect_role"]   = $role; // Pass role to view for redirect
    ...
    public function edit($id)
    {
        ...
        $date                     = $employee_payroll['year'] . "-" . str_pad($employee_payroll['month'], 2, '0', STR_PAD_LEFT);
        $data['result']           = $searchEmployee;
    ...
    public function payslip()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($checkForUpdate == true) {
                ...
                // Get search parameters to redirect back to same filtered page
                $redirect_month = $month;
                $redirect_year = $year;
                $redirect_role = $this->input->post("redirect_role");
                
                // Set success message
                $this->session->set_flashdata("msg", "<div class='alert alert-success text-center'>" . $this->lang->line('success_message') . "</div>");
                
                // Redirect back to search page with same parameters
                if (!empty($redirect_role)) {
                    redirect('admin/payroll/search/' . $redirect_month . '/' . $redirect_year . '/' . $redirect_role);
                } else {
                    redirect('admin/payroll/search/' . $redirect_month . '/' . $redirect_year);
                }
            } else {

                // Get search parameters for redirect
                $redirect_month = $month;
                $redirect_year = $year;
                $redirect_role = $this->input->post("redirect_role");
    ...
    public function search($month, $year, $role = '')
    {
        ...
        $data["month"]          = $month;
        $data["year"]           = $year;
        $data['month_numeric']  = $month; // Keep numeric
        $data['year_numeric']   = $year;  // Keep numeric
        $data['sch_setting']    = $this->sch_setting_detail;
        $data["payroll_status"] = $this->payroll_status;
        $data["resultlist"]     = $searchEmployee;
        $data["payment_mode"]   = $this->payment_mode;
    ...
    public function paymentRecord()
    {
        ...
        $data["month"]      = isset($nepaliMonths[$month]) ? $nepaliMonths[$month] : $month;
        $data["year"]       = $this->convertToNepaliDigits((string)$year);
