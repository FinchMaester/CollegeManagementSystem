...
class Currency extends Admin_Controller
{
    ...
    public function changeactive()
    {
        ...
        if (!empty($currency) && $this->session->has_userdata('admin')) {
    ...
    public function change_currency()
    {
        $currency_id = (int) $this->input->post('currency_id');
        if ($currency_id < 1) {
            echo json_encode(array('status' => 0, 'message' => 'Invalid currency.'));
            return;
        }
        $currency = $this->currency_model->get($currency_id);
        if (empty($currency)) {
            echo json_encode(array('status' => 0, 'message' => 'Currency not found.'));
            return;
        }
        ...
        echo json_encode(array('status' => 1, 'message' => $this->lang->line('currency_changed_successfully')));
    }
