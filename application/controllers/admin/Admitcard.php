...
class Admitcard extends Admin_Controller
{
    ...
    public function view()
    {
        ...
        if (is_string($student_ids)) {
            $student_ids = array_filter(array_map('trim', explode(',', $student_ids)));
        } elseif (!is_array($student_ids)) {
            $student_ids = array();
        }
        $class_id   = $this->input->post('class_id');
        $section_id = $this->input->post('section_id');
    
        // uses your aliased fetch (students_roll_no / students_symbol_no)
        $data['student_details'] = $this->fetch_students_with_roll_symbol($student_ids, $class_id, $section_id);
    
        // ---- DEBUG TO VIEW / CONSOLE ----
        // hit endpoint with ?debug=1  (or send POST debug=1) to enable
        $data['__admitcard_debug']       = (int)($this->input->get('debug') || $this->input->post('debug'));
        $data['__admitcard_debug_sql']   = $this->db->last_query();                           // last SELECT SQL
        $data['__admitcard_debug_first'] = !empty($data['student_details']) ? $data['student_details'][0] : null; // first row
        // ---------------------------------
    
        // defaults to avoid notices
        $data['exam_subjects'] = isset($data['exam_subjects']) ? $data['exam_subjects'] : array();
        $data['exam']          = isset($data['exam']) ? $data['exam'] : (object) array('use_exam_roll_no' => 1);
    ...
    /**
     * Fetch students; alias students table columns to avoid collisions.
     * Also logs the final SQL for verification.
     */
    private function fetch_students_with_roll_symbol($student_ids = array(), $class_id = null, $section_id = null)
    {
        $this->db->select('
            students.id,
            students.firstname,
            ...
            /* ONLY from students table */
            students.roll_no   AS students_roll_no,
            students.symbol_no AS students_symbol_no,
            /* Keep session roll if ever needed, but with a different alias */
            student_session.roll_no AS session_roll_no,
            classes.class,
            sections.section
        ');
        ...
        if (!empty($student_ids))  { $this->db->where_in('students.id', $student_ids); }
        if (!empty($class_id))     { $this->db->where('student_session.class_id', $class_id); }
        if (!empty($section_id))   { $this->db->where('student_session.section_id', $section_id); }
    
        $this->db->order_by('students.firstname', 'ASC');
        $q = $this->db->get();
    
        // Log the executed SQL for debugging
        log_message('debug', '[Admitcard:fetch_students_with_roll_symbol] SQL: ' . $this->db->last_query());
