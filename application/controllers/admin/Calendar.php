...
class Calendar extends Admin_Controller
{
    ...
    public function addtodo()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            // Convert Y-m-d format to school date format if needed
            $task_date = $this->input->post('task_date');
            if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $task_date)) {
                // Date is in Y-m-d format, convert to school format
                $parts = explode('-', $task_date);
                $year = $parts[0];
                $month = $parts[1];
                $day = $parts[2];
                $school_format = $this->customlib->getSchoolDateFormat();
                
                if ($school_format == 'd-m-Y') {
                    $task_date = $day . '-' . $month . '-' . $year;
                } elseif ($school_format == 'd/m/Y') {
                    $task_date = $day . '/' . $month . '/' . $year;
                } elseif ($school_format == 'm-d-Y') {
                    $task_date = $month . '-' . $day . '-' . $year;
                } elseif ($school_format == 'm/d/Y') {
                    $task_date = $month . '/' . $day . '/' . $year;
                } elseif ($school_format == 'Y/m/d') {
            ...
            $start_date        = date('Y-m-d H:i:s', $this->customlib->datetostrtotime($task_date));
