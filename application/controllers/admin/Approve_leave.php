...
class approve_leave extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if (isset($_POST['program_id']) && $_POST['program_id'] != '') {
            $data['program_id'] = $program_id = $_POST['program_id'];
        }
    
        $this->form_validation->set_rules('class_id', $this->lang->line('class'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('section_id', $this->lang->line('section'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('program_id', $this->lang->line('program'), 'trim|xss_clean');
    
        if ($this->form_validation->run() == false) {
            $listaudit = $this->apply_leave_model->get(null, null, null);
        } else {
            $listaudit = $this->apply_leave_model->get(null, $class_id, $section_id, $program_id);
        }
    ...
    public function get_details()
    {
        ...
        if ($can_edit == 0) {
            ...
            // Add program_id to the response
            $data['program_id'] = $this->get_program_id_by_class_section($data['class_id'], $data['section_id']);
            
            $data['leave_status'] = $data['status'];
            $data['from_date'] = $data['from_date'];
            $data['to_date'] =   $data['to_date'];
            $data['apply_date'] =$data['apply_date'];
        }
        echo json_encode($data);
    }
    
    private function get_program_id_by_class_section($class_id, $section_id)
    {
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $student_session_id = $this->apply_leave_model->get_studentsessionId($_POST['class_id'], $_POST['section_id'], $_POST['student']);
    ...
    public function searchByClassSectionProgram() {
        $section_id = $this->input->post('section_id');
        $program_id = $this->input->post('program_id');
        $class_id = $this->input->post('class_id');
        $selected_student_id = $this->input->post('selected_student_id'); // Optional parameter for editing
        
        // Call the model method with all three parameters
        $resultlist = $this->student_model->searchByClassSectionProgram($class_id, $section_id, $program_id);
        
        $data['resultlist'] = $resultlist;
        $data['sch_setting'] = $this->sch_setting_detail;
        $data['selected_student_id'] = $selected_student_id; // Pass to view for maintaining selection
