...
class Leaverequest extends Admin_Controller
{
    ...
    public function leaverequest()
    {
        ...
        // Add staff_id to data for the view
    ...
    public function countLeave($id)
    {
        ...
        foreach ($alloted_leavetype as $key => $value) {
            ...
            $data[$i]['approve_leave'] = $count_leaves[$i]['approve_leave'] ? $count_leaves[$i]['approve_leave'] : 0;
    ...
    public function leaveRecord()
{
    ...
    $result->leavefrom    = $result->leave_from; 
    $result->date         = $result->date;       
    $result->leaveto      = $result->leave_to;   
    ...
    public function addLeave()
    {
        ...
        $applied_date = $this->input->post("applieddate"); // Nepali date in YYYY-MM-DD format
        ...
        if ($this->form_validation->run() == false) {
            ...
            // Get the Nepali dates directly from the form
            $leavefrom    = $this->input->post('leave_from_date'); // Nepali date in YYYY-MM-DD
            $leaveto      = $this->input->post('leave_to_date');   // Nepali date in YYYY-MM-DD
            $applied_date_nepali = $this->input->post('applieddate'); // Nepali date in YYYY-MM-DD
            
            $applied_by   = $this->customlib->getStaffID();
            
            // For calculating days difference, convert to English dates temporarily
            $leavefrom_english = date("Y-m-d", $this->customlib->datetostrtotime($leavefrom));
            $leaveto_english   = date("Y-m-d", $this->customlib->datetostrtotime($leaveto));
            $leave_days   = $this->dateDifference($leavefrom_english, $leaveto_english);
            ...
            if ($total_remain >= $leave_days) {
                ...
                $data = array(
                    'staff_id'        => $staff_id, 
                    'date'           => $applied_date_nepali, // Store Nepali date directly
                    'leave_days'      => $leave_days, 
                    'leave_type_id'   => $leavetype, 
                    'leave_from'      => $leavefrom, // Store Nepali date directly
                    'leave_to'        => $leaveto,   // Store Nepali date directly
    ...
    {
        $userdata     = $this->customlib->getUserData();
        $applied_date = $this->input->post("applieddate"); // Nepali date
        $leave_from_date = $this->input->post("leave_from_date"); // Nepali date
        $leave_to_date = $this->input->post("leave_to_date"); // Nepali date
        $leavetype    = $this->input->post("leave_type");
        $reason       = $this->input->post("reason");
        $remark       = '';
        $status       = 'pending';
        ...
        if ($this->form_validation->run() == false) {
            ...
            // Store Nepali dates directly (no conversion)
            $leavefrom = $leave_from_date; // Storing Nepali date as-is
            $leaveto   = $leave_to_date;   // Storing Nepali date as-is
            $applied_date_nepali = $applied_date; // Storing Nepali date as-is
    
            $staff_id     = $userdata["id"];
            $applied_by   = $this->customlib->getStaffID();
            
            // If you need to calculate days difference, you might need to convert to English dates temporarily
            $leavefrom_english = date("Y-m-d", $this->customlib->datetostrtotime($leave_from_date));
            $leaveto_english   = date("Y-m-d", $this->customlib->datetostrtotime($leave_to_date));
            $leave_days   = $this->dateDifference($leavefrom_english, $leaveto_english);
            ...
            if ($total_remain >= $leave_days) {
                ...
                if (!empty($request_id)) {
                    $data = array(
                        'id' => $request_id,
                        'staff_id'         => $staff_id,
                        'date'             => $applied_date_nepali, // Storing Nepali date
                        'leave_type_id'    => $leavetype,
                        'leave_days'       => $leave_days,
                        'leave_from'      => $leavefrom, // Storing Nepali date
                        'leave_to'        => $leaveto,   // Storing Nepali date
                        'employee_remark' => $reason,
                        'status'          => $status,
                        'admin_remark'    => $remark,
                        'applied_by'      => $applied_by,
                ...
                $message       = $this->input->post('reason') . '<br>' . 
                               $this->lang->line('apply_date') . ': ' . $applied_date_nepali . '<br>' . 
                               $this->lang->line('from_date') . ': ' . $leavefrom . '<br>' . 
                               $this->lang->line('to_date') . ': ' . $leaveto;
