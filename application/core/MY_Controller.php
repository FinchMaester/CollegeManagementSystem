...
class MY_Controller extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        // Auto-detect and set session based on Nepali calendar
        // This ensures the system always opens with the correct session
        // regardless of where it was logged out
        if ($this->config->item('installed') == true) {
            try {
                $this->setting_model->autoDetectAndSetSession();
            } catch (Exception $e) {
                // Log error and set debug info for console
                log_message('error', 'Auto-session detection failed: ' . $e->getMessage());
                
                // Set error debug info for console
                $error_debug = array(
                    'status' => 'error',
                    'error' => $e->getMessage(),
                    'ad_date' => date('Y-m-d'),
                    'ad_year' => (int)date('Y'),
                    'ad_month' => (int)date('m'),
                    'target_session' => '2082-83' // Default fallback
                );
                $this->session->set_userdata('auto_session_debug', $error_debug);
            }
        }
    }
