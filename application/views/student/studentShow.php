...
$currency_symbol = (isset($currency_symbol) && $currency_symbol !== '') ? $currency_symbol : (isset($admin_session['currency_symbol']) ? $admin_session['currency_symbol'] : $this->customlib->getSchoolCurrencyFormat());
$class_section = (isset($class_section) && is_array($class_section)) ? $class_section : array();
$studentlistbysection = (isset($studentlistbysection) && is_array($studentlistbysection)) ? $studentlistbysection : array();
$session = isset($session) ? $session : '';
$category_list = (isset($category_list) && (is_array($category_list) || $category_list instanceof \Traversable)) ? $category_list : array();
$attendencetypeslist = (isset($attendencetypeslist) && (is_array($attendencetypeslist) || $attendencetypeslist instanceof \Traversable)) ? $attendencetypeslist : array();
$session_year_start = isset($session_year_start) ? $session_year_start : date('Y-m-01');
            ...
            <aside class="studentsidebar">
                <div class="stutop" id="">
                    ...
                    <div class="studentsidetopfixed">
                        <p class="classtap"><?php echo isset($student['class']) ? htmlspecialchars($student['class']) : ''; ?> <a href="#" data-toggle="control-sidebar" class="studentsideclose"><i class="fa fa-times"></i></a></p>
                        <ul class="nav nav-justified studenttaps">
                            <?php foreach ($class_section as $skey => $svalue) {
    ?>
                                <li <?php
if (isset($student['section_id']) && $student['section_id'] == $svalue["section_id"]) {
        echo "class='active'";
    }
    ?> ><a href="#section<?php echo $svalue["section_id"] ?>" data-toggle="tab"><?php echo isset($svalue['section']) ? htmlspecialchars($svalue['section']) : ''; ?></a></li>
                                <?php }?>
                        </ul>
                    </div>
                    <!-- Tab panes -->
...
if (isset($student['section_id']) && $student['section_id'] == $snvalue["section_id"]) {
        echo "active";
    }
    ?>" id="section<?php echo $snvalue["section_id"]; ?>">
                                 <?php
foreach ($studentlistbysection as $stkey => $stvalue) {
        if (isset($stvalue['section_id'], $snvalue['section_id']) && $stvalue['section_id'] == $snvalue["section_id"]) {
...
if (!empty($stvalue["image"])) {
    echo base_url('uploads/student_images/' . basename($stvalue["image"]));

                } else {
                    if (isset($student['gender']) && $student['gender'] == 'Female') {
                        echo (isset($this->media_storage) && is_object($this->media_storage)) ? $this->media_storage->getImageURL("student_images/default_female.jpg") : base_url('uploads/student_images/default_female.jpg');
                    } else {
                        echo (isset($this->media_storage) && is_object($this->media_storage)) ? $this->media_storage->getImageURL("student_images/default_male.jpg") : base_url('uploads/student_images/default_male.jpg');
                    }
                }
                ?>" alt="">
                                                <?php }?>
...
if (!empty($student["image"]) && file_exists(FCPATH . 'uploads/student_images/' . basename($student["image"]))) {
    $image_url = base_url('uploads/student_images/' . basename($student["image"]));
    } else {
        if (isset($student['gender']) && $student['gender'] == 'Female') {
            if (file_exists(FCPATH . 'uploads/student_images/default_female.jpg')) {
                $image_url = base_url('uploads/student_images/default_female.jpg');
            } else {
                $image_url = base_url('uploads/no_image.png');
            }
        } else {
            if (file_exists(FCPATH . 'uploads/student_images/default_male.jpg')) {
                $image_url = base_url('uploads/student_images/default_male.jpg');
            } else {
                $image_url = base_url('uploads/no_image.png');
            }
        }
    ...
    ?>
                     ...
                     <div class="box-body box-profile pt0">
                        <ul class="list-group list-group-unbordered">
                            <?php
if (isset($student['is_active']) && $student['is_active'] == 'no') {
    ?>
                                <li class="list-group-item listnoback">
                                    <b><?php echo $this->lang->line('disable_reason'); ?></b> <span class="pull-right text-aqua"><?php echo isset($reason_data['reason']) ? $reason_data['reason'] : ''; ?></span>
                                </li>
                                <li class="list-group-item listnoback">
                                     <b><?php echo $this->lang->line('disable_note'); ?></b> <span class="pull-right text-aqua"><?php echo isset($student['dis_note']) ? $student['dis_note'] : ''; ?></span>
                                </li>
                                 <li class="list-group-item listnoback">
                                    <b><?php echo $this->lang->line('disable_date'); ?></b> <span class="pull-right text-aqua"><?php echo (isset($student['disable_at']) && !empty($student['disable_at'])) ? date($this->customlib->getSchoolDateFormat(), $this->customlib->dateyyyymmddTodateformat($student['disable_at'])) : ''; ?></span>
                                </li>
                            <?php }?>
...
if ($sch_setting->roll_no) {
    ?>
                            ...
                            <li class="list-group-item listnoback border0">
                                <b><?php echo $this->lang->line('class'); ?></b> <a class="pull-right text-aqua"><?php echo (isset($student['class']) ? $student['class'] : '') . (isset($session) && $session !== '' ? ' (' . $session . ')' : ''); ?></a>
                            </li>
                            <li class="list-group-item listnoback">
                                <b><?php echo $this->lang->line('section'); ?></b> <a class="pull-right text-aqua"><?php echo isset($student['section']) ? $student['section'] : ''; ?></a>
                            </li>
                            ...
                            <li class="list-group-item listnoback">
                                ...
                                <?php if (file_exists(FCPATH . "uploads/student_id_card/barcodes/" . $student['admission_no'] . ".png")) {
?>
                                   <a class="pull-right text-aqua">
                                    <img class="h-36" src="<?php echo base_url('uploads/student_id_card/barcodes/' . $student['admission_no'] . '.png')
...
if ($this->module_lib->hasModule('behaviour_records')) {
    if ($this->rbac->hasPrivilege('behaviour_records_assign_incident', 'can_view')) {
            ...
            ?>
                            <li class="list-group-item listnoback">
                                <b><?php echo $this->lang->line('behaviour_score'); ?></b> <a class="pull-right text-aqua"><?php echo isset($student['total_points']) ? $student['total_points'] : '&mdash;'; ?></a>
                            </li>
                        <?php
...
if (!empty($sibling_value->image) && file_exists(FCPATH . 'uploads/student_images/' . basename($sibling_value->image))) {
    echo base_url('uploads/student_images/' . basename($sibling_value->image));
        } else {
            if (isset($sibling_value->gender) && $sibling_value->gender == 'Female') {
                if (file_exists(FCPATH . 'uploads/student_images/default_female.jpg')) {
                    echo base_url('uploads/student_images/default_female.jpg');
                } else {
                    echo base_url('uploads/no_image.png');
                }
            } else {
                if (file_exists(FCPATH . 'uploads/student_images/default_male.jpg')) {
                                 ...
                                 <div class="box-body pt0">
                                    <div class="no-padding">
                                        <ul class="list-group list-group-unbordered">
                                            ...
                                            <li class="list-group-item listnoback">
                                                <b><?php echo $this->lang->line('rte'); ?></b> <a class="pull-right text-aqua"><?php echo $sibling_value->rte; ?></a>
                                            </li>
                                            <?php }?>
                                            <li class="list-group-item listnoback">
                                            ...
                                            <li class="list-group-item listnoback">
                                                <b><?php echo $this->lang->line('barcode'); ?></b>
                                                <?php if (file_exists("./student_id_card/barcodes/" . $sibling_value->admission_no . ".png")) {?>
                                                <a class="pull-right text-aqua">
                                                    <img class="h-36" src="<?php echo (isset($this->media_storage) && is_object($this->media_storage)) ? $this->media_storage->getImageURL('uploads/student_id_card/barcodes/' . $sibling_value->admission_no) : base_url('uploads/student_id_card/barcodes/' . $sibling_value->admission_no . '.png'); ?>" width="auto" height="auto"/></a>
                                                <?php }?>
                                            </li>
                                            <?php }?>
                                        </ul>
...
if ($this->rbac->hasPrivilege('disable_student', 'can_view')) {
        ?>
                                ...
                                <li class="pull-right">
                                    <a style="cursor: pointer;" href="#" class="text-red js-disable-student" data-student-id="<?php echo (int) $student['id']; ?>" data-toggle="tooltip" data-placement="bottom" title="<?php echo $this->lang->line("disable"); ?>">
	...
	<?php if ($this->module_lib->hasActive('fees_collection') && !empty($student['student_session_id'])) { ?>
     <li class="pull-right">
                                    <a href="<?php echo site_url('studentfee/addfee/' . (int) $student['student_session_id']); ?>" data-toggle="tooltip" data-placement="bottom" title="<?php echo $this->lang->line('collect_fees'); ?>"><b><?php echo $currency_symbol; ?>  </b>
                                    </a>
                                </li>
	<?php } ?>							
                            <?php
if ($this->rbac->hasPrivilege('student', 'can_edit')) {
    ...
    ?>
                            <?php if ($this->module_lib->hasActive('fees_collection') && !empty($student['student_session_id']) && $this->rbac->hasPrivilege('collect_fees', 'can_view')) { ?>
                            <li class="pull-right">
                                <a href="<?php echo site_url('studentfee/addfee/' . (int) $student['student_session_id']); ?>" data-toggle="tooltip" data-placement="bottom" title="Collect outstanding fees (student disabled)"><b><?php echo $currency_symbol; ?>  </b></a>
                            </li>
                            <?php } ?>
                            <li class="pull-right">
                                <a href="#" onclick="(function(){ if(!confirm('<?php echo addslashes($this->lang->line('are_you_sure_you_want_to_enable_this_record')); ?>')) return; $.ajax({ type:'post', url: '<?php echo base_url(); ?>student/enablestudent/<?php echo (int)$student['id']; ?>', data:{student_id:'<?php echo (int)$student['id']; ?>'}, dataType:'json', success:function(){ window.location.reload(true); }, error:function(){ alert('Enable failed'); } }); })()" class="text-green" data-placement="bottom" data-toggle="tooltip" title="<?php echo $this->lang->line('enable'); ?>">
...
if (!empty($student['admission_date'])) {
        echo date($student['admission_date']);
    }
    ?></td>
                                            </tr>
                                            <?php }?>
...
?>
                            ...
                            <div class="tshadow mb25 bozero">
                                ...
                                <div class="table-responsive around10 pt0">
                                    <table class="table3 table-hover table-striped tmb0"><tbody>
    ...
    <td rowspan="3">
        <img class="profile-user-img img-responsive img-rounded" src="<?php 
            if (!empty($student["father_pic"]) && file_exists(FCPATH . 'uploads/student_images/' . basename($student["father_pic"]))) {
                echo base_url('uploads/student_images/' . basename($student["father_pic"]));
            } else {
                echo base_url('uploads/no_image.png');
            }
        ?>" alt="Father Photo">
...
if (!empty($student["mother_pic"]) && file_exists(FCPATH . 'uploads/student_images/' . basename($student["mother_pic"]))) {
    echo base_url('uploads/student_images/' . basename($student["mother_pic"]));
        } else {
            echo base_url('uploads/no_image.png');
        }
        ?>" alt="Mother Photo"></td>
        ...
        ?><img class="profile-user-img img-responsive img-rounded" src="<?php
if (!empty($student["guardian_pic"]) && file_exists(FCPATH . 'uploads/student_images/' . basename($student["guardian_pic"]))) {
            echo base_url('uploads/student_images/' . basename($student["guardian_pic"]));
        } else {
            echo base_url('uploads/no_image.png');
        }
        ?>" alt="Guardian Photo"> <?php }?></td>
                                        </tr>
        ...
        foreach ($assignstudent as $assignstudent_value) {
            ...
            ?>
                                                    <tr class="<?php echo $pointclass; ?>">
                                                        ...
                                                        <td> <?php echo $assignstudent_value['staff_name'] . ' ' . $assignstudent_value['staff_surname'] . $staff_id ?></td>
                                                        <td >
    ...
    foreach ($student_doc as $value) {
        ...
        ?>
                                                        <tr>
                                                            ...
                                                            <td><?php echo (isset($this->media_storage) && is_object($this->media_storage)) ? $this->media_storage->fileview($value['doc']) : htmlspecialchars(basename($value['doc'])); ?></td>
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <?php 
                        $display = true;
                        $nepaliMonths = array('Baisakh', 'Jestha', 'Ashad', 'Shrawan', 'Bhadra', 'Ashoj', 'Kartik', 'Mangsir', 'Poush', 'Magh', 'Falgun', 'Chaitra');
                        $monthIndex = 0;
                        foreach ($nepaliMonths as $monthName) { 
                        ?>
                            <td>
                                <?php 
                                if ($display) {
                                ...
                                if ($monthIndex < 11) { // Not the last month
                                    $mm = date('m', strtotime('+1 month', strtotime($start_year)));
...
?>
                        <div class="tab-pane" id="exam">
                            ...
                            <div id="visible">
                                <center><h4 class="hide" id="exam_student_name"><?php echo $this->customlib->getFullName($student['firstname'], $student['middlename'], $student['lastname'], $sch_setting->middlename, $sch_setting->lastname); ?> (<?php echo $student["class"]; ?>)  </h4></center>
...
if ($total_exams) {
                     ...
                     <div class="description-block"><h5 class="description-header"><?php echo $this->lang->line('total_obtain_marks'); ?> :  <span class="description-text"><?php echo $exam_get_total; ?></span></h5>
                     </div>
                     </div>
                     <?php
} elseif ($exam_value->exam_type == "gpa") {
                            ...
                            <div id='timeline_hide_show' class="row">
                                ...
                                <div class=" col-md-12">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input class="valign-sub" id="visible_check" checked="checked" name="visible_check" value="yes" placeholder="" type="checkbox"   />
                                    </div>
                                </div>
                              </div>
                            </div>
...
?>
                ...
                <div class="box-footer">
                    <div class="pull-right paddA10">
                        <button type="submit" class="btn btn-info pull-right disable-save-btn" data-loading-text="<i class='fa fa-spinner fa-spin '></i> Please wait"><?php echo $this->lang->line('save'); ?></button>
                    </div>
                </div>
            </form>
        </div>
...
(function ($) {
    ...
    function disable_student(id) {
        if (confirm(<?php echo json_encode($this->lang->line('are_you_sure_you_want_to_disable_this_student')); ?>)) {
    ...
    window.disable_student = disable_student;

    $(document).on('click', '.js-disable-student', function (e) {
        e.preventDefault();
        disable_student($(this).data('student-id'));
    });

    $("#disable_form").on('submit', function (e) {
        e.preventDefault();
        var $btn = $(this).find(".disable-save-btn");

        $.ajax({
            url: <?php echo json_encode(site_url('student/disable_reason')); ?>,
            type: "POST",
            data: new FormData(this),
            dataType: 'json',
            contentType: false,
...
<script type="text/javascript">
    ...
    function enable(id, status, role) {
        if (confirm("<?php echo $this->lang->line('are_you_sure_you_want_to_enable_this_record'); ?>")) {
            ...
            $.ajax({
                ...
                dataType: "json",
                success: function (response) {
                    var userid = (response && response.data && response.data.id) ? response.data.id : (response && response.id ? response.id : null);
                    if (userid) {
    ...
    // Make sure inline `onclick="enable(...)"` can always resolve the symbol.
    // Some bundling/DOM edge-cases can prevent function declarations from becoming global bindings.
    ...
    function send_password() {
        ...
        var student_id = <?php echo json_encode(isset($student['id']) ? (string) $student['id'] : ''); ?>;
        var username = <?php echo json_encode(isset($student['username']) ? $student['username'] : ''); ?>;
        var password = <?php echo json_encode(isset($student['password']) ? $student['password'] : ''); ?>;
        var contact_no = <?php echo json_encode(isset($student['mobileno']) ? $student['mobileno'] : ''); ?>;
        var email = <?php echo json_encode(isset($student['email']) ? $student['email'] : ''); ?>;
        var admission_no = <?php echo json_encode(isset($student['admission_no']) ? $student['admission_no'] : ''); ?>;
    ...
    function send_parent_password() {
        var base_url = <?php echo json_encode(base_url()); ?>;
        var student_id = <?php echo json_encode(isset($student['id']) ? (string) $student['id'] : ''); ?>;
        var username = <?php echo json_encode(isset($guardian_credential['username']) ? $guardian_credential['username'] : ''); ?>;
        var password = <?php echo json_encode(isset($guardian_credential['password']) ? $guardian_credential['password'] : ''); ?>;
        var contact_no = <?php echo json_encode(isset($student['guardian_phone']) ? $student['guardian_phone'] : ''); ?>;
        var email = <?php echo json_encode(isset($student['guardian_email']) ? $student['guardian_email'] : ''); ?>;
        var admission_no = <?php echo json_encode(isset($student['admission_no']) ? $student['admission_no'] : ''); ?>;
...
// this is the id of the form
    ...
    $("#form1").on('submit', function(e) {
        ...
        var title = $('input[name="first_title"]').val();
        var fileInput = $('input[name="first_doc"]')[0];
        
        console.log('Student ID:', studentId);
        console.log('Title:', title);
        console.log('File input:', fileInput);
        console.log('File:', fileInput ? fileInput.files[0] : 'No file');
        ...
        $.ajax({
            ...
            error: function(xhr, status, error) {
                ...
                if (xhr.status === 500) {
                    errorMessage = "Server Error (500): " + (xhr.responseText ? xhr.responseText.substring(0, 200) : "Internal server error");
...
function testAjax() {
    $.ajax({
        url: "<?php echo site_url('student/test_endpoint') ?>",
        type: "POST",
        dataType: 'json',
        success: function(res) {
        ...
        error: function(xhr, status, error) {
            console.log('Test failed:', xhr.responseText);
        }
    });
}
</script>
