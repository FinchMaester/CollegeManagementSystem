    ...
    <div class="form-horizontal">
        <div class="col-lg-12">
            <div class="form-group row">
                <label for="collect_date" class="col-sm-3 control-label"><?php echo $this->lang->line('date'); ?> <small class="req"> *</small></label>
                <div class="col-sm-9">
                    <input id="collect_date" name="collected_date" placeholder="" type="text" class="form-control date_fee nepali-date-picker" value="" autocomplete="off">
...
<?php
if (!isset($discount_map) || !is_array($discount_map)) {
    $discount_map = array();
}
?>
<ul class="fees-list fees-list-in-box">
    ...
    if (($fee_value->due_date != "0000-00-00" && $fee_value->due_date != NULL) && (strtotime($fee_value->due_date) < strtotime(date('Y-m-d'))) && $amount_to_be_pay > 0) {
        ...
        if ($amount_to_be_pay > 0) {
            ...
            if (!empty($discount_map) && isset($discount_map[$fee_value->fee_groups_feetype_id])) {
                $line_discounts = $discount_map[$fee_value->fee_groups_feetype_id];
            }
            $feetype_id_row = isset($fee_value->feetype_id) ? (int) $fee_value->feetype_id : 0;
            ?>

            <li class="item bulk-fee-item" data-row="<?php echo $row_counter; ?>" data-balance="<?php echo (float) $amount_to_be_pay; ?>" data-feetype-id="<?php echo $feetype_id_row; ?>" data-fgf-id="<?php echo (int) $fee_value->fee_groups_feetype_id; ?>">
                ...
                <input name="fee_groups_feetype_id_<?php echo $row_counter; ?>" type="hidden" value="<?php echo $fee_value->fee_groups_feetype_id; ?>">
                 <input name="fee_groups_feetype_fine_amount_<?php echo $row_counter; ?>" type="hidden" class="bulk-fee-fine" value="<?php echo $fees_fine_amount; ?>">
                <input name="fee_amount_<?php echo $row_counter; ?>" type="hidden" class="bulk-fee-amount-net" value="<?php echo $amount_to_be_pay; ?>">
                <input name="amount_discount_<?php echo $row_counter; ?>" type="hidden" class="bulk-amount-discount" value="0">
                ...
                <div class="product-info">
                    <a href="#"  onclick="return false;" class="product-title">                       
                        ...
                        ?>
                        <span class="pull-right bulk-line-pay"><?php echo  $currency_symbol.amountFormat((float) $amount_to_be_pay, 2, '.', ''); ?></span></a>
...
if($fine_amount_status){
    ...
    <?php
                         ...
                         ?>
                <?php if (!empty($line_discounts)) { ?>
                <div class="row bulk-discount-row" style="margin-top:8px;">
                    <div class="col-sm-7">
                        <label class="small"><?php echo $this->lang->line('fees_discount'); ?></label>
                        <select class="form-control input-sm bulk-fee-discount" name="student_fees_discount_id_<?php echo $row_counter; ?>" data-row="<?php echo $row_counter; ?>">
                            <option value=""><?php echo $this->lang->line('select'); ?></option>
                            <?php foreach ($line_discounts as $discount) {
                                $sfd_id = isset($discount->student_fees_discount_id) ? (int) $discount->student_fees_discount_id : 0;
                                $label  = $discount->code;
                                if ($discount->type === 'percentage') {
                                    $label .= ' (' . $discount->percentage . '%)';
                                } else {
                                    $label .= ' (' . amountFormat($discount->amount) . ')';
                                }
                                ?>
                                <option value="<?php echo $sfd_id; ?>"
                                    data-type="<?php echo htmlspecialchars($discount->type, ENT_QUOTES, 'UTF-8'); ?>"
                                    data-percentage="<?php echo (float) $discount->percentage; ?>"
                                    data-disamount="<?php echo (float) $discount->amount; ?>">
                    ...
                    <div class="col-sm-5">
                        <label class="small"><?php echo $this->lang->line('discount'); ?> (<?php echo $currency_symbol; ?>)</label>
                        <input type="text" class="form-control input-sm bulk-discount-display" readonly value="0">
                    </div>
                </div>
                <?php } elseif ($fee_value->fee_category === 'fees') { ?>
                <p class="text-muted small bulk-no-discount-hint" style="margin:8px 0 0;">No discount available for this fee line (already used on this line, not assigned, or wrong fee type).</p>
                <?php } ?>
                </div>
            </li>
...
<div class="row collect_grp_fees" style="margin-top:6px;">
    <div class="col-md-8">
        <span class="pull-right text-muted small bulk-total-discount-label" style="display:none;"><?php echo $this->lang->line('discount'); ?></span>
        <span class="pull-right" style="display:block; margin-top:4px;">
            <?php echo $this->lang->line('total_pay'); ?>
        </span>
    </div>
    <div class="col-md-4">
        <span class="pull-right text-muted small bulk-total-discount-amount" style="display:none;">0</span>
        <span class="pull-right bulk-total-pay" style="display:block; margin-top:4px;">
