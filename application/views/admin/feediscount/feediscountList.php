                ...
                <div class="col-md-4">
                    ...
                    <div class="col-sm-6">
                         <label for="exampleInputEmail1"><?php echo $this->lang->line('amount').' ('. $currency_symbol.')'; ?></label><small class="req"> *</small>
                                ...
                                <div class="form-group">
                                    <label for="feetype_ids"><?php echo $this->lang->line('fee_type'); ?> <small class="text-muted">(optional)</small></label>
                                    <select class="form-control" id="feetype_ids" name="feetype_ids[]" multiple="multiple" size="6">
                                        <?php
                                        $posted_feetypes = $this->input->post('feetype_ids');
                                        if (!is_array($posted_feetypes)) {
                                            $posted_feetypes = array();
                                        }
                                        if (!empty($feetype_list)) {
                                            foreach ($feetype_list as $feetype_item) {
                                                $ft_id = (int) $feetype_item['id'];
                                                $ft_label = $feetype_item['type'];
                                                if (!empty($feetype_item['code'])) {
                                                    $ft_label .= ' (' . $feetype_item['code'] . ')';
                                                }
                                                $sel = in_array($ft_id, array_map('intval', $posted_feetypes)) ? 'selected' : '';
                                                echo '<option value="' . $ft_id . '" ' . $sel . '>' . htmlspecialchars($ft_label, ENT_QUOTES, 'UTF-8') . '</option>';
                                            }
                                        }
                                        ?>
                                    </select>
                                    <p class="help-block" style="margin-top:6px;">Leave empty to allow this discount on <strong>all</strong> fee types. Select one or more to restrict where it can be used when collecting fees.</p>
                                </div>
                                ...
                                <tbody>
                                    ...
                                    foreach ($feediscountList as $feediscount) {
                                        ...
                                        <tr>
                                            ...
                                            <td class="mailbox-name">
                                                <?php
                                                $applicable = isset($feediscount['applicable_summary']) ? $feediscount['applicable_summary'] : 'all';
                                                echo ($applicable === 'all')
                                                    ? '<span class="label label-default">All fee types</span>'
                                                    : htmlspecialchars($applicable, ENT_QUOTES, 'UTF-8');
                                                ?>
                                            </td>
