    ...
    <div class="col-md-12 column">
        ...
        <form method="POST" action="<?php echo site_url('admin/timetable/savetimetable'); ?>" id="form_<?php echo $day; ?>" class="commentForm autoscroll">
            ...
            <div class="box-footer">
                <div id="save_button_area">
                    <?php if ($this->rbac->hasPrivilege('class_timetable', 'can_edit')) { ?>
                        <button type="button" class="btn btn-primary btn-sm pull-right" onclick="saveTimetable()">
...
<script type="text/javascript">
    ...
    function saveTimetable() {
        var form = $('#form_<?php echo $day; ?>');
        var formData = new FormData(form[0]);
       
        $.ajax({
            type: 'POST',
            url: baseurl + "admin/timetable/savegroup",
            data: formData,
            processData: false,
            contentType: false,
            dataType: 'json',
            ...
            success: function(response) {
                if (response.status == 1) {
                    ...
                    getGroupdata($('.tab-pane.active'), null, $('.nav-tabs .active a').data());
                } else {
                    var message = "";
