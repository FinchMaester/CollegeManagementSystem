        ...
        <div class="row">
            <div class="col-md-6">
                <div class="box box-primary">
                    <div class="box-body">
                        <style>
                        /* Remove yellow highlight from today's cell in FullCalendar */
                        #calendar_event .fc-today,
                        #calendar_event td.fc-today,
                        #calendar_event .fc-state-highlight { background: transparent !important; }
                        </style>
                            ...
                            <table class="table table-striped table-bordered table-hover example">
                                <thead>
                                    <tr class="white-space-nowrap">
                                        ...
                                        <th><?php echo $this->lang->line('class'); ?><br><?php echo $this->lang->line('section'); ?></th>
...
?>
                        ...
                        <div class="form-group col-md-4">
                            <label><?php echo $this->lang->line('event_from_date'); ?></label><small class="req"> *</small>                          
                                <input type="text" autocomplete="off" id="from_date" name="from_date" class="form-control nepali-datepicker" >                            
                        </div>
                         <div class="form-group col-md-4">
                            <label><?php echo $this->lang->line('event_to_date'); ?></label><small class="req"> *</small>                          
                                <input type="text" autocomplete="off" id="to_date" name="to_date" class="form-control nepali-datepicker">
...
$(function(){
    // Ensure only Nepali datepicker is active on these fields
    if ($.fn.datepicker) {
        ["#from_date","#to_date"].forEach(function(sel){
            try { $(sel).datepicker('destroy'); } catch(e) {}
            try { $(sel).datepicker('remove'); } catch(e) {}
            try { $(sel).off('.datepicker'); } catch(e) {}
            try { $(sel).data('datepicker', null); } catch(e) {}
        });
    }
    $('#newevent').on('shown.bs.modal', function(){
        if ($.fn.datepicker) {
            try { $("#from_date").datepicker('destroy'); } catch(e) {}
            try { $("#from_date").datepicker('remove'); } catch(e) {}
            try { $("#from_date").off('.datepicker'); } catch(e) {}
            try { $("#from_date").data('datepicker', null); } catch(e) {}
            try { $("#to_date").datepicker('destroy'); } catch(e) {}
            try { $("#to_date").datepicker('remove'); } catch(e) {}
            try { $("#to_date").off('.datepicker'); } catch(e) {}
            try { $("#to_date").data('datepicker', null); } catch(e) {}
        }
    });

    if ($.fn.fullCalendar) {
        var $cal = $('#calendar_event');
        var nepMonths = ['बैशाख','जेठ','असार','साउन','भदौ','असोज','कार्तिक','मंसिर','पुष','माघ','फागुन','चैत्र'];
        function renderNepaliTitle() {
            try {
                var m = $cal.fullCalendar('getDate'); // moment
                var ad = m.format('YYYY-MM-DD');
                if (window.NepaliFunctions && typeof NepaliFunctions.AD2BS === 'function') {
                    var bs = NepaliFunctions.AD2BS(ad); // 'YYYY-MM-DD'
                    var parts = bs.split('-');
                    var y = parts[0];
                    var mn = parseInt(parts[1], 10) - 1;
                    var title = (nepMonths[mn] || '') + ' ' + y;
                    $('.fc-center h2').text(title);
                } else {
                    // Fallback: just switch locale (Nepali text for Gregorian)
                    $cal.fullCalendar('option', 'locale', 'ne');
                }
            } catch(e) {}
        }

        function renderBSDayNumbers() {
            if (!(window.NepaliFunctions && typeof NepaliFunctions.AD2BS === 'function')) return;
            try {
                // Month view: replace each cell's day number with BS day
                $('.fc-day-top').each(function(){
                    var $top = $(this);
                    var date = $top.parent().data('date');
                    if (!date) return;
                    var bs = NepaliFunctions.AD2BS(date);
                    var d = parseInt(bs.split('-')[2], 10);
                    var $num = $top.find('.fc-day-number');
                    if ($num.length) { $num.text(d); }
                });
            } catch(e) { /* ignore */ }
        }
        ...
        try {
            if ($cal.data('fullCalendar')) {
                ...
                $cal.fullCalendar({
                    locale: 'ne',
                    viewRender: function(){ renderNepaliTitle(); renderBSDayNumbers(); },
                    eventAfterAllRender: function(){ renderBSDayNumbers(); }
                });
            }
            // Also update on navigation clicks
            $(document).on('click', '.fc-prev-button, .fc-next-button, .fc-today-button', function(){
                setTimeout(function(){ renderNepaliTitle(); renderBSDayNumbers(); }, 0);
            });
            renderNepaliTitle();
            renderBSDayNumbers();
        } catch(e) {}
...
?>";
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (newsection) {
                    try {
                        var JSONObject = (typeof newsection === 'string') ? JSON.parse(newsection) : newsection;
                        for (var i = 0, l = JSONObject.length; i < l; i++) {
                            $('input[name="user[]"][value="' + JSONObject[i] + '"]').prop("checked", true);
                        }
                    } catch (e) {
                        // ignore bad preselection
                    }
                }
...
<script type="application/javascript">  
 ...
 // Use Nepali (BS) datepicker: set values directly in YYYY-MM-DD (BS) if provided
    if (res.from_date) { $("#from_date").val(res.from_date); }
    if (res.to_date) { $("#to_date").val(res.to_date); }
    try { $("#from_date").datepicker('destroy'); } catch(e) {}
    try { $("#to_date").datepicker('destroy'); } catch(e) {}
