            ...
            <div class="col-md-12">
                ...
                <div class="box box-primary">
                    <div class="box-header ptbnull">
                        ...
                        <button type="button" id="uploadBtn" class="btn btn-sm btn-primary" autocomplete="off"><i class="fa fa-cloud-upload"></i> <?php echo $this->lang->line('upload'); ?></button>
                        </div><!-- /.box-tools -->
                        <?php }?>
                    </div><!-- /.box-header -->
                    <div class="box-body">
    ...
    <div class="documents_sidebar">
        <div class="d-preview text-center p-2">
            <div class="d-preview-position">
                <img src="<?php echo base_url('backend/images/upload-file.png'); ?>"  <?php if ($this->rbac->hasPrivilege('upload_content', 'can_add')) {?> class="add_image" <?php }?>>
...
<script type="text/javascript">
    ...
    // Handle image loading with comprehensive debugging
    $(document).ready(function() {
        console.log('=== IMAGE LOADING DEBUG START ===');
        console.log('Base URL (JS):', baseurl);
        console.log('Branch Base URL:', branch_base_url);
        ...
        $('.content-image').each(function(index) {
            var $img = $(this);
            var debugData = $img.data('debug') || {};
            
            // Log base URL detection info
            if (debugData.base_url_debug) {
                console.log('Base URL Detection for Image ' + (index + 1) + ':', {
                    stored_base_url: debugData.base_url_debug.stored_base_url,
                    current_base_url: debugData.base_url_debug.current_base_url,
                    stored_host: debugData.base_url_debug.stored_host,
                    current_host: debugData.base_url_debug.current_host,
                    selected_base_url: debugData.base_url_debug.selected_base_url,
                    source: debugData.base_url_debug.source,
                    reason: debugData.base_url_debug.stored_host === debugData.base_url_debug.current_host ? 
                        'Hosts match - using stored URL' : 
                        'Hosts differ - using current URL (local/server mismatch)'
                });
            }
            ...
            console.log('Image ' + (index + 1) + ':', {
                src: $img.attr('src'),
                alt: $img.attr('alt'),
                base_url_used: debugData.base_url || 'N/A',
                base_url_source: debugData.base_url_source || 'N/A',
                thumb_path: debugData.thumb_path || 'N/A',
                thumb_name: debugData.thumb_name || 'N/A',
                final_url: debugData.final_url || 'N/A',
            ...
            testImg.onerror = function() {
                ...
                console.error('  Check if file exists at:', $img.attr('src'));
            };
            testImg.src = $img.attr('src');
        });
        
        // Handle image load events
        $('.img-wrap img, .content-image').on('load', function() {
            var $img = $(this);
            console.log('Image loaded:', $img.attr('src'));
            $img.addClass('loaded');
        }).on('error', function() {
            var $img = $(this);
            console.error('Image error:', $img.attr('src'));
        }).each(function() {
            // If image is already cached, mark as loaded immediately
            if (this.complete && this.naturalWidth > 0) {
                $(this).addClass('loaded');
                console.log('Image already cached:', $(this).attr('src'));
            }
        });
    ...
    $(document).ready(function (e) {
        ...
        // Initialize single-delete modal separately to prevent flickering
        $('#single-delete').modal({
            backdrop: 'static',
            keyboard: false,
            show:false
...
$(document).on('submit','form#fileupload',function(e) {
    ...
    // Debug: Log form data
    console.group('=== UPLOAD DEBUG START ===');
    console.log('URL:', url);
    ...
    $.ajax({
            ...
            beforeSend: function (xhr) {
                console.log('Before Send - XHR Status:', xhr.status);
                $this.button('loading');
            },
            success: function (data, textStatus, xhr)
            {
              ...
              if (!data.status) {
                   ...
                   var message = "";
                   $.each(data.error, function (index, value) {
                       console.error('  Error [' + index + ']:', value);
                       message += value;
                   });
                   errorMsg(message);
                   } else {
            ...
            if (xhr.status === 0) {
                ...
                console.error('ERROR: Server error - check server logs');
...
// Handle delete file click - prevent default and open modal manually
$(document).on('click', '.delete_file', function(e) {
    e.preventDefault();
    e.stopPropagation();
    var recordId = $(this).data('record-id');
    var name = $(this).data('name');
    $('#single-delete .title').text(name);
    $('#single-delete .btn-ok').data('recordId', recordId);
    $('#single-delete').modal('show');
});

$('#single-delete').on('show.bs.modal', function(e) {
    // This will be triggered when modal is shown programmatically
    // Data is already set by the click handler above
});
