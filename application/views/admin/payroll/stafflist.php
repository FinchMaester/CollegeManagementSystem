...
?>
    ...
    <div class="form-group">
        ...
        <select autofocus="" id="month_select" name="month" class="form-control" >
            <option value=""><?php echo $this->lang->line('select'); ?></option>
            <?php
            $month_selected = isset($month) ? $month : date("n");
            foreach ($monthlist as $m_key => $month_value) {
                $selected = '';
                if (isset($_POST['month']) && $_POST['month'] == $m_key) {
                    $selected = 'selected="selected"';
                } elseif (!isset($_POST['month']) && $m_key == $month_selected) {
                    $selected = 'selected="selected"';
                }
                ?>
                <option value="<?php echo $m_key ?>" <?php echo $selected; ?>>
    ...
    <div class="form-group">
        ...
        <select autofocus="" id="year_select" name="year" class="form-control" >
            <option value=""><?php echo $this->lang->line('select'); ?></option>
            <?php
            if (isset($yearlist) && !empty($yearlist)) {
                foreach ($yearlist as $year_key => $year_value) {
                    $selected = '';
                    if (isset($_POST['year']) && $_POST['year'] == $year_key) {
                        $selected = 'selected="selected"';
                    }
                    ?>
                    <option value="<?php echo $year_key; ?>" <?php echo $selected; ?>>
                        <?php echo $year_value; ?>
                    </option>
                    <?php
                }
            } else {
                // Fallback if yearlist not available
                for ($y = 2076; $y <= 2083; $y++) {
                    $selected = ($year == $y) ? 'selected="selected"' : '';
                    ?>
                    <option value="<?php echo $y; ?>" <?php echo $selected; ?>>
                        २०७६, २०७७, २०७८, २०७९, २०८०, २०८१, २०८२, २०८३
                    </option>
                    <?php
                }
            }
...
if ($this->rbac->hasPrivilege('staff_payroll', 'can_delete')) {
                ?>
                                                        <a class="btn btn-default btn-xs" onclick="return confirm('<?php echo $this->lang->line("are_you_sure_you_want_to_revert_this_record") ?>')"href="<?php echo base_url() . "admin/payroll/revertpayroll/" . $staff["payslip_id"] . "/" . $month_numeric . "/" . $year_numeric . "/" . $role_selected ?>" title="<?php echo $this->lang->line("revert") ?>">
...
if ($this->rbac->hasPrivilege('staff_payroll', 'can_delete')) {
                ?>
                                                        <a href="<?php echo base_url() ?>admin/payroll/deletepayroll/<?php echo $staff["payslip_id"] . "/" . $month_numeric . "/" . $year_numeric . "/" . $role_selected ?>"class="btn btn-default btn-xs" onclick="return confirm('<?php echo $this->lang->line("are_you_sure_you_want_to_revert_this_record") ?>')" title="<?php echo $this->lang->line("revert") ?>">
                                                            <i class="fa fa-undo"> </i>
                                                        </a>
                <?php
}
...
if ($this->rbac->hasPrivilege('staff_payroll', 'can_add')) {
                ?>
                                                        <a class="btn btn-primary btn-xs checkbox-toggle edit_setting" role="button" href="<?php echo base_url() . "admin/payroll/create/" . $month_numeric . "/" . $year_numeric . "/" . $staff["id"] . (!empty($role_selected) ? "/" . urlencode($role_selected) : "") ?>"><?php echo $this->lang->line('generate_payroll'); ?></a>
                                            <?php }?>
                                                </td>
                                        <?php }?>
                                        </tr>
...
?>
                        ...
                        <div class="form-group col-xs-12 col-sm-12 col-md-12 col-lg-6">               
                            ...
                            <input type="text" name="payment_date" id="payment_date" class="form-control nepali-datepicker" value="">
                        </div>
                        <div class="form-group col-xs-12 col-sm-12 col-md-12 col-lg-6">
...
<script type="text/javascript">
    function getRecord(id, year) {
        ...
        // Get month from the dropdown - NOW IT'S NUMERIC
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $('#monthid').val(result.month + '-' + result.year);
            },
            error: function(xhr, status, error) {
                console.error('AJAX Error:', error);
                console.error('Response:', xhr.responseText);
                alert('Failed to fetch payment record. Check console for details.');
            }
        });
...
<script type="text/javascript">
$(document).ready(function() {
    // Initialize Nepali datepicker and set today's Nepali date
    $('#payment_date').nepaliDatePicker({
        ndpYear: true,
        ndpMonth: true,
        ndpYearCount: 10,
    ...
    // Auto-fill with today's Nepali date when modal opens
    $('#proceedtopay').on('shown.bs.modal', function () {
        if ($('#payment_date').val() === '') {
            // Get today's Nepali date from the datepicker
            var today = NepaliFunctions.GetCurrentBsDate();
            var nepaliDate = today.year + '-' + 
                           String(today.month).padStart(2, '0') + '-' + 
                           String(today.day).padStart(2, '0');
            $('#payment_date').val(nepaliDate);
        }
    });
});
</script>