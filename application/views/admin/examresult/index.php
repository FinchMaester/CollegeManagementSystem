...
if (!empty($studentList) && !empty($subjectList)) {
    foreach ($studentList as $__sv) {
        ...
        foreach ($subjectList as $__subj) {
            ...
            if (strtoupper($grade) === 'NG' || ($__exam_type === 'gpa' && $grade === '-')) $any_ng = true;
    ...
    <?php foreach ($subjectList as $subject_value): ?>
        <th>
            <?php
                ...
                if (isset($exam_details->exam_group_type) && $exam_details->exam_group_type === 'gpa') {
                    echo "<br/>(" . $this->lang->line('marks') . " / " . $this->lang->line('credit_hours') . " / " . $this->lang->line('grade_point') . " / " . $this->lang->line('grade') . ")";
                } else {
...
<?php if (isset($exam_details->exam_group_type) && $exam_details->exam_group_type !== 'gpa'): ?>
                                                    <th><?php echo $this->lang->line('percent') ?: 'Percentage'; ?> (%)</th>
<?php endif; ?>
                        ...
                        <?php else: ?>
                            <?php
                                ...
                                if ($exam_details->exam_group_type == "gpa") {
                                    ...
                                    $subj_letter = grade_from_points((float) $gp);
                                    $ch = (float) $subject_value->credit_hours;
                                    echo number_format((float)$result->get_marks, 2, '.', '') . "<br/><small>"
                                        . htmlspecialchars($this->lang->line('credit_hours') ?: 'CH') . ': ' . number_format($ch, 2, '.', '')
                                        . ' · ' . htmlspecialchars($this->lang->line('grade_point') ?: 'GP') . ': ' . number_format((float)$gp, 2, '.', '')
                                        . ' · [' . htmlspecialchars($subj_letter) . ']</small>';
                                } else {
...
<?php if (isset($exam_details->exam_group_type) && $exam_details->exam_group_type !== 'gpa'): ?>
            <td><?php echo ($total_marks > 0) ? number_format((float)(($get_marks * 100.0) / $total_marks), 2, '.', '') . '%' : '--'; ?></td>
<?php endif; ?>
                        ...
                        if ($exam_details->exam_group_type === 'gpa') {
                            if ($total_credit_hour > 0) {
                                ...
                                if ($final_grade === 'NG') {
                                    ...
                                    echo number_format((float)$total_quality_point, 2, '.', '') . '/'
                                        . number_format((float)$total_credit_hour, 2, '.', '') . '='
                                        . number_format($final_gpa, 2, '.', '') . ' [' . htmlspecialchars($final_grade) . ']';
                                }
                            } else {
                                echo "";
                            }
