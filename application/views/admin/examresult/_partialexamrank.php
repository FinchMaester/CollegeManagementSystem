<?php
/* Helpers (must be defined before use in loop below) */
if (!function_exists('getSubjectMarks')) {
...
if (!function_exists('_partialexamrank_in_range')) {
function _partialexamrank_in_range($p, $a, $b) {
    $low = min((float)$a, (float)$b);
    $high = max((float)$a, (float)$b);
    return ($p + 1e-9) >= $low && ($p - 1e-9) <= $high;
}
}
if (!function_exists('get_ExamGrade')) {
function get_ExamGrade($exam_grades, $percentage)
{
    if ($percentage === null) return '-';
    if (!empty($exam_grades)) {
        foreach ($exam_grades as $g) {
            if (_partialexamrank_in_range($percentage, $g->mark_from, $g->mark_upto)) return $g->name;
        }
    }
    return '-';
}
...
function findGradePoints($exam_grades, $percentage)
{
    ...
    if (!empty($exam_grades)) {
        foreach ($exam_grades as $g) {
            if (_partialexamrank_in_range($percentage, $g->mark_from, $g->mark_upto)) return isset($g->point) ? (float)$g->point : 0.0;
        }
    }
    return 0;
}
}
/** Letter from cumulative GPA value (same as exam result index / marksheet) */
if (!function_exists('_partialexamrank_grade_from_points')) {
function _partialexamrank_grade_from_points($p) {
    $p = (float) $p;
    if ($p >= 3.61) return 'A+';
    if ($p >= 3.21) return 'A';
    if ($p >= 2.81) return 'B+';
    ...
    foreach ($studentList as $student_key => $student_value) {
        ...
        if (!empty($subjectList)) {
            ...
            foreach ($subjectList as $subject_key => $subject_value) {
                ...
                if ($result) {
                    $no_subject_result = 1;
                    $result_absent = (strtolower($result->attendence ?? '') === 'absent');
                    if ($exam_details->exam_group_type == "gpa") {
                        if ($result_absent) {
                            $student_exam_status = 0; // Absent: do not calculate GPA, treat as fail
                        } else {
                            ...
                            $percentage_grade    = ($result->max_marks > 0) ? (($result->get_marks * 100) / $result->max_marks) : 0;
                            $point               = findGradePoints($exam_grades, $percentage_grade);
                            $total_quality_point = $total_quality_point + ($point * $subject_credit_hour);
                            $subject_grade = get_ExamGrade($exam_grades, $percentage_grade);
                            if (strtoupper($subject_grade) === 'NG' || $subject_grade === '-') $student_exam_status = 0;
                        }
                    } else {
                        if ($result_absent) {
...
// Rank by total obtained marks (descending), per Exam Type Flow Analysis
uasort($pass_student, function ($a, $b) {
    return (int) ($b->get_marks <=> $a->get_marks);
});
uasort($fail_student, function ($a, $b) {
    return (int) ($b->get_marks <=> $a->get_marks);
});
                    ...
                    <input type="hidden" name="exam_group_class_batch_exam_student_id_<?php echo $student_f_value->exam_group_class_batch_exam_student_id; ?>" value="<?php echo $rank_increment; ?>">
                       ...
                       <td><?php echo $student_f_value->student_name; ?>  </td>
                        <td>
                         <?php
                         if ($exam_details->exam_group_type == "gpa") {
                if (!$student_f_value->student_exam_status) {
                    echo $this->lang->line('fail') ?: 'Fail';
                } elseif ($student_f_value->total_credit_hour > 0) {
                    $gpa_val = $student_f_value->total_quality_point / $student_f_value->total_credit_hour;
                    $exam_qulity_point = number_format($gpa_val, 2, '.', '');
                    $letter = _partialexamrank_grade_from_points($gpa_val);
                    if ($letter === 'NG') {
                        echo $this->lang->line('fail') ?: 'Fail';
                    } else {
                        echo number_format((float)$student_f_value->total_quality_point, 2, '.', '') . '/'
                            . number_format((float)$student_f_value->total_credit_hour, 2, '.', '') . '='
                            . $exam_qulity_point . ' [' . htmlspecialchars($letter) . ']';
                    }
                } else {
                    echo "--";
                }
                         } elseif ($exam_details->exam_group_type === 'basic_system') {
                            // General Purpose (Pass/Fail): Percent(%) column already shows %; Result is only Pass/Fail
                            echo $student_f_value->student_exam_status
                                ? ($this->lang->line('pass') ?: 'Pass')
                                : ($this->lang->line('fail') ?: 'Fail');
                         } else {
                            // Other non-GPA: show Fail or Percentage% (GP) [Grade]
                            if (!$student_f_value->student_exam_status) {
                                echo $this->lang->line('fail') ?: 'Fail';
                            } else {
                                $pct = $student_f_value->total_percentage;
                                $grade = get_ExamGrade($exam_grades, $pct);
                                $gp = findGradePoints($exam_grades, $pct);
                                echo number_format((float)$pct, 2, '.', '') . '% (' . number_format((float)$gp, 2, '.', '') . ') [' . htmlspecialchars($grade) . ']';
                            }
                         }
                             ?>                                
...
if($exam_details->exam_group_type != "gpa"){
   ...
   <td>
                        <?php
                            ...
                            echo number_format((float)$total_percentage, 2, '.', '');
                        ?>
                       </td>
    <?php
}
