...
<?php if (isset($subject_detail) && is_object($subject_detail)) { ?>
...
if (isset($resultlist) && !empty($resultlist)) {
    ...
    <div class="alert alert-info text-center">
        ...
        <?php if (!empty($no_record_hint)) { ?>
            <p class="mt10 mb0"><strong><?php echo htmlspecialchars($no_record_hint); ?></strong></p>
            <?php if (!empty($linked_levels) && is_array($linked_levels)) {
                $parts = array();
                foreach ($linked_levels as $lev) {
                    $section = isset($lev['section_name']) ? $lev['section_name'] : '';
                    $program = isset($lev['program_name']) && $lev['program_name'] !== '' ? $lev['program_name'] : '';
                    $class   = isset($lev['class_name']) ? $lev['class_name'] : '';
                    $parts[] = $section . ($program !== '' ? ' / ' . $program : '') . ' / ' . $class;
                }
                if (!empty($parts)) { ?>
                    <p class="mt5 mb0"><?php echo $this->lang->line('linked_students_are_in') ?: 'Linked students are in'; ?>: <strong><?php echo htmlspecialchars(implode('; ', array_unique($parts))); ?></strong>.</p>
                    <p class="mt5 mb0"><?php echo $this->lang->line('select_those_and_search') ?: 'Select the same Faculty, Program and Academic Level above, then click Search.'; ?></p>
                <?php }
            } ?>
        <?php } ?>
    </div>
