...
/**
 * Language IDs shown in the admin header switcher (from sch_settings.languages).
 * Ensures English + Nepali appear when their language packs exist on disk.
 */
function get_header_language_ids()
{
    $CI = &get_instance();
    $ids = array();
    $settings = $CI->setting_model->get();
    if (!empty($settings[0]['languages'])) {
        $decoded = json_decode($settings[0]['languages']);
        if (is_array($decoded)) {
            foreach ($decoded as $lang_id) {
                $lang_id = (int) $lang_id;
                if ($lang_id > 0) {
                    $ids[] = $lang_id;
                }
    ...
    $ensure = array(4 => 'English', 53 => 'Nepali');
    foreach ($ensure as $lang_id => $folder) {
        if (!in_array($lang_id, $ids, true) && is_dir(APPPATH . 'language/' . $folder)) {
            $ids[] = $lang_id;
        }
    }
...
/**
 * Add captcha fields to front CMS form-builder configs when enabled.
 */
function inject_form_builder_captcha($CI)
{
    foreach (array('contact_us', 'complain') as $form_key) {
        if (!$CI->captchalib->is_captcha($form_key)) {
            continue;
        }

        $form = $CI->config->item($form_key);
        if (empty($form)) {
            continue;
        }
        ...
        array_insert($form, 'submit', array('captcha' => $captcha));
        $CI->config->set_item($form_key, $form);
    }
}
...
function convertCurrencyFormatToBaseAmount($amount)
{
    $CI              = &get_instance();
    $currency_price  = $CI->customlib->getSchoolCurrencyPrice();
    log_message('debug', 'convertCurrencyFormatToBaseAmount: Input amount=' . $amount . ', currency_price=' . $currency_price);
    // Ensure amount is treated as a numeric value before division
    $amount = (is_numeric($amount) ? (float)$amount : 0.0);
   
    // Prevent division by zero
    if ((float)$currency_price === 0.0) {
        log_message('error', 'convertCurrencyFormatToBaseAmount: Division by zero avoided. currency_price is 0.');
        return 0.0;
    }


    $calculated_amount = floatval($amount / $currency_price);
    log_message('debug', 'convertCurrencyFormatToBaseAmount: Calculated amount=' . $calculated_amount);
