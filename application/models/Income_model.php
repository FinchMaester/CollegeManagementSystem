...
class Income_model extends My_Model
{
    ...
    public function search($text = null, $start_date = null, $end_date = null)
    {
        if (!empty($text)) {
            $this->datatables
                ...
                ->orderable('income.name,CAST(income.invoice_no AS UNSIGNED),income.date,income_head.income_category,income.amount')
                ->join("income_head", "income.income_head_id = income_head.id")
                ->like('income.name', $text)
                ->from('income');

        } else {
            // Build DataTables query only — do not run other queries on $this->db here; the Datatables
            // library shares the same query builder, and extra selects/gets corrupt state and can break
            // JSON output on production (Invalid JSON / SQL errors).
            $this->datatables
                ->select('income.id,income.date,income.name,income.invoice_no,income.amount,income.documents,income.note,income_head.income_category,income.income_head_id')
                ->searchable('income.name,income.invoice_no,income.date,income_head.income_category,income.amount')
                ->orderable('income.name,CAST(income.invoice_no AS UNSIGNED),income.date,income_head.income_category,income.amount')
    ...
    public function searchincomegroup($start_date = null, $end_date = null, $head_id = null)
    {
        $this->datatables
            ...
            ->orderable('income_head.income_category,income.id,income.name,income.date,CAST(income.invoice_no AS UNSIGNED)')
            ->join('income_head', 'income.income_head_id = income_head.id')
    ...
    public function getincomelist()
    {
        $this->datatables
            ...
            ->orderable('income.name,income.note,CAST(income.invoice_no AS UNSIGNED),income.date,income_head.income_category,income.amount')
            ->join("income_head", "income.income_head_id = income_head.id")
            ->sort('income.id', 'desc')
            ->from('income');
        return $this->datatables->generate('json');
