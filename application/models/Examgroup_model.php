...
class Examgroup_model extends MY_Model
{
    ...
    public function getExamByID($id = null)
    {
        ...
        $result = $query->row();
        // Ensure subject_group_id is included in the result
        if ($result && !isset($result->subject_group_id)) {
            $result->subject_group_id = null;
        }
        return $result;
    }
    ...
    {
        if (isset($data['id'])) {
            // Update exam_group_class_batch_exams
            $this->db->where('id', $data['id']);
            ...
            // Update exams table as well (by name and sesion_id)
            $exams_data = array(
                'name' => isset($data['exam']) ? $data['exam'] : null,
                'sesion_id' => isset($data['session_id']) ? $data['session_id'] : null,
                'is_active' => isset($data['is_active']) ? ($data['is_active'] ? 'yes' : 'no') : 'no',
                'updated_at' => date('Y-m-d H:i:s')
            );
            $this->db->where('name', $exams_data['name']);
            $this->db->where('sesion_id', $exams_data['sesion_id']);
            $this->db->update('exams', $exams_data);
        } else {
            ...
            $record_id = $insert_id;
            $this->log($message, $record_id, $action);
            // Insert into exams table as well, but only if not already present
            $exams_data = array(
                'name' => isset($data['exam']) ? $data['exam'] : null,
                'sesion_id' => isset($data['session_id']) ? $data['session_id'] : null,
                'is_active' => isset($data['is_active']) ? ($data['is_active'] ? 'yes' : 'no') : 'no',
            ...
            if ($query->num_rows() == 0) {
                $this->db->insert('exams', $exams_data);
            }
        }
    }
    ...
    public function getExamByExamGroup($id, $is_active = false)
    {
        ...
        // Convert sessions to BS format (Nepali)
        if (!empty($results)) {
            ...
            foreach ($results as $result) {
                if (isset($result->session)) {
                    $result->session = $this->customlib->convertSessionToBSIfNeeded($result->session);
                }
            }
        }
