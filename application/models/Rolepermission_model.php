...
class Rolepermission_model extends CI_Model
{
    ...
    public function getPermissionByRoleandCategory($role_id, $category)
    {
        ...
        $this->db->join('permission_category', 'permission_category.id=roles_permissions.perm_cat_id');
        $this->db->where('roles_permissions.role_id', $role_id);
        $this->db->where('permission_category.short_code', trim($category));
        $query = $this->db->get();
        
        // Debug: Log query details for troubleshooting
        if (isset($_SERVER['HTTP_HOST']) && strpos($_SERVER['HTTP_HOST'], 'test.') !== false) {
            log_message('debug', 'Permission Query - Role ID: ' . $role_id . ', Category: ' . trim($category));
            log_message('debug', 'Permission Query SQL: ' . $this->db->last_query());
            log_message('debug', 'Permission Query Rows: ' . $query->num_rows());
        }
    ...
    /**
     * Fetch all permission_category rows for a set of role IDs.
     *
     * @param int[] $role_ids
     * @return array<int, array> rows from roles_permissions joined with permission_category
     */
    public function getPermissionsByRoles(array $role_ids)
    {
        $ids = array();
        ...
        $this->db->from('roles_permissions');
        $this->db->join('permission_category', 'permission_category.id=roles_permissions.perm_cat_id');
        $this->db->where_in('roles_permissions.role_id', array_values($ids));
        $query = $this->db->get();
