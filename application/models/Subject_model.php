...
class Subject_model extends MY_Model {
    ...
    function check_data_exists($data) {
        ...
        // Ensure uniqueness is scoped by type (Theory vs Practical)
        if (isset($data['type']) && $data['type'] !== '') {
            $this->db->where('type', $data['type']);
        } else {
            // Fallback to posted type if not provided explicitly
            $posted_type = $this->input->post('type');
            if (!empty($posted_type)) {
                $this->db->where('type', $posted_type);
            }
        }
        // Exclude current record when editing
        if (isset($data['exclude_id']) && !empty($data['exclude_id'])) {
            $this->db->where('id !=', (int)$data['exclude_id']);
        }
    ...
    function check_code_exists($data) {
        ...
        // Ensure code uniqueness is scoped by type (Theory vs Practical)
        if (isset($data['type']) && $data['type'] !== '') {
            $this->db->where('type', $data['type']);
        } else {
            // Fallback to posted type if not provided explicitly
