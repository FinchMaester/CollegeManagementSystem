...
class Classteacher_model extends MY_Model
{
    ...
    public function teacherByClassSection($class_id, $section_id, $program_id = null)
    {
        $this->db->select('staff.*')->from('class_teacher');
        ...
        if ($program_id !== null) {
            $this->db->where('class_teacher.program_id', $program_id);
        }
    ...
    public function delete($class_id, $section_id, $program_id, $array = array())
    {
        ...
        if ($program_id === 0 || $program_id === '0') {
            $this->db->group_start();
            $this->db->where('program_id', 0);
            $this->db->or_where('program_id IS NULL');
            $this->db->group_end();
        } else {
        ...
        // Debugging line: Dump the compiled delete query
        // echo $this->db->last_query();
        // exit();
        // var_dump($this->db->get_compiled_delete('class_teacher')); exit();
    }
