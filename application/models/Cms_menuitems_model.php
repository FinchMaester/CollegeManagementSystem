...
class Cms_menuitems_model extends MY_Model
{
    ...
    public function getMenus($menu_id, $parent = 0, $spacing = '', $user_tree_array = '')
    {
        ...
        foreach ($result as $r_key => $obj) {
            $obj = $this->normalize_menu_item_links($obj);
    ...
    /**
     * Demo installs store front-menu links like http://yourschoolurl.com/online_admission
     * as external URLs. Rewrite those (and same-path links on any host) to internal routes.
     */
    private function normalize_menu_item_links($obj)
    {
        $link = isset($obj->ext_url_link) ? trim((string) $obj->ext_url_link) : '';
        $routes = array(
            'online_admission' => 16,
            'online_course'    => 13,
            'examresult'       => 10,
        ...
        if ($link !== '' && preg_match('/yourschoolurl\.com/i', $link)) {
            $use_internal = true;
        }

        if ($link !== '') {
            foreach ($routes as $route_name => $suffix_len) {
                if (substr($link, -$suffix_len) === $route_name) {
                    $route        = $route_name;
                    $use_internal = true;
                    break;
                }
        ...
        if ($route === null && (int) $obj->page_id === 0) {
            if (strcasecmp(trim((string) $obj->menu), 'Online Admission') === 0
                || (string) $obj->slug === 'admssion') {
                $route        = 'online_admission';
                $use_internal = true;
            }
        }

        if ($use_internal && $route !== null) {
            $obj->ext_url      = null;
            $obj->ext_url_link = null;
            $obj->page_url     = $route;
            $obj->page_slug    = $route;
        }
