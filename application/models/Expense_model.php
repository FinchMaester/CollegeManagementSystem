    ...
    public function search($text = null, $start_date = null, $end_date = null)
{
    if (!empty($text)) {
        $this->datatables
            ...
            ->orderable('expenses.name,CAST(expenses.invoice_no AS UNSIGNED),exp_category,date,expenses.amount')
    ...
    public function getexpenselist($id = null)
    {
        $this->datatables
            ...
            ->orderable('expenses.name,expenses.note,CAST(expenses.invoice_no AS UNSIGNED),expenses.date,expense_head.exp_category,expenses.amount')
            ->join("expense_head", "expenses.exp_head_id = expense_head.id")
