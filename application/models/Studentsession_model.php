...
class Studentsession_model extends CI_Model
{
    ...
    public function searchStudentsBySession($student_session_id = null)
    {
        $this->db->select('students.admission_no,students.roll_no,student_session.session_id, student_session.class_id, student_session.section_id,student_session.id,student_session.student_id,classes.class,sections.section,
            students.firstname,students.middlename,students.lastname,students.admission_no,students.mobileno,students.dob,students.guardian_name,students.father_name,students.guardian_phone,students.guardian_email,students.email')->from('student_session');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('students', 'students.id = student_session.student_id');
    ...
    public function updatePromote($update_array)
    {
        ...
        $existing = $this->db->get('student_session')->row();


       
        if ($existing) {
            if (isset($update_array['is_alumni'])) {
                unset($update_array['is_alumni']);
            }
            ...
            $current_record = $this->db->get('student_session')->row();
           
            if ($current_record) {
                $graduation_record = array(
                    'session_id' => $update_array['session_id'],
                    'student_id' => $update_array['student_id'],
                    'class_id' => $update_array['class_id'],
                    'section_id' => $update_array['section_id'],
                    'program_id' => $update_array['program_id'],
                    'is_graduated' => $update_array['is_graduated'],
                    'is_alumni' => $update_array['is_alumni'],
                    'is_active' => 'yes',
                    'created_at' => date('Y-m-d H:i:s'),
                    'updated_at' => date('Y-m-d'),
                    'hostel_room_id' => $current_record->hostel_room_id,
                    'vehroute_id' => $current_record->vehroute_id,
                    'route_pickup_point_id' => $current_record->route_pickup_point_id,
                    'transport_fees' => $current_record->transport_fees,
                    'fees_discount' => $current_record->fees_discount,
                    'is_leave' => 0,
                    'default_login' => $current_record->default_login,
                    'is_pass' => 1,
                    'is_fail' => 0
                );
               
                $this->db->insert('student_session', $graduation_record);
                $insert_id = $this->db->insert_id();
               
                log_message('debug', 'New graduation record created with ID: ' . $insert_id);
                log_message('debug', 'Insert query: ' . $this->db->last_query());
               
                return $insert_id > 0;
            } else {
                log_message('error', 'No current student record found for student ID: ' . $update_array['student_id']);
                return false;
            }
        }
    }
    ...
    public function searchMultiStudentByClassSection($class_id = null, $section_id = null, $program_id = null)
    {
        // Get students using the Student_model method
        $students = $this->student_model->searchByClassSectionWithSession($class_id, $section_id, null, $program_id);
    
        if (!empty($students)) {
            foreach ($students as $student_key => $student_value) {
                // Fix: Add proper SELECT fields and JOIN with programs table
                $this->db->select('
                    student_session.*,
                    classes.class,
                    sections.section,
                ...
                $this->db->where('student_session.student_id', $student_value['id']);
                ...
                if (!$query) {
                    log_message('error', 'DB error in searchMultiStudentByClassSection: ' . $this->db->last_query());
                    log_message('error', 'DB error: ' . print_r($this->db->error(), true));
                    $students[$student_key]['student_sessions'] = [];
                } else {
    ...
    public function getDropoutsCount()
    {
        $this->db->select('COUNT(DISTINCT student_id) as count');
        $this->db->from('student_session');
