...
class Studentfeemaster_model extends MY_Model
{
    ...
    public function assign_bulk_fees($fee_session_group,$student_session_id,$delete_fee_session_group)
    {
        ...
        if ($CI->student_model->isStudentSessionAccountDisabled($student_session_id)) {
            return false;
        }
    ...
    {
        // Validate student_session_id
        if (empty($student_session_id) || !is_numeric($student_session_id)) {
            return array();
        }
        ...
        if (!empty($result)) {
            foreach ($result as $result_key => $result_value) {
                ...
                if ($result_value->is_system != 0 && !empty($result_value->fees)) {
                    $result_value->fees[0]->amount = $result_value->amount;
                }
            }
        }
    ...
    {
        $fee_category_saved      = isset($data['fee_category']) ? $data['fee_category'] : 'fees';
        $fee_groups_feetype_saved = isset($data['fee_groups_feetype_id']) ? (int) $data['fee_groups_feetype_id'] : 0;
        ...
        if ($q->num_rows() > 0) {
            ...
            if ($student_fees_discount_id != null) {
                $payment_ref = $row->id . "//" . $inv_no;
                $this->feediscount_model->markDiscountUsedAfterPayment(
                    $student_fees_discount_id,
                    $fee_groups_feetype_saved,
                    $fee_category_saved,
                    $payment_ref,
                    $desc
                );
    ...
    public function getFeeCollectionReport($start_date, $end_date, $feetype_id= null,$received_by = null, $group = null, $class_id = null, $section_id = null, $program_id = null)
    {
        ...
        // Use both runtime session and configured system session to avoid missing
        // newly collected records when session_array drifts from sch_settings.
        $configured_session_id = 0;
        $configured_session_q  = $this->db->query("SELECT `sch_settings`.`session_id` AS `configured_session_id` FROM `sch_settings` LIMIT 1");
        if ($configured_session_q && $configured_session_q->num_rows() > 0) {
            $configured_session_row = $configured_session_q->row();
            $configured_session_id  = (int) $configured_session_row->configured_session_id;
        }
        $session_ids = array_unique(array_filter(array((int) $this->current_session, $configured_session_id)));
        if (count($session_ids) > 1) {
            $this->db->where_in('fee_groups_feetype.session_id', $session_ids);
            $this->db->where_in('student_session.session_id', $session_ids);
        } else {
        ...
        if ($program_id != null) {
            // Backward compatibility: older student_session rows may not have program_id set,
            // while students.program_id is populated.
            $this->db->where('(student_session.program_id = ' . $this->db->escape($program_id) . ' OR (student_session.program_id IS NULL AND students.program_id = ' . $this->db->escape($program_id) . '))', null, false);
        }
    ...
    public function findObjectAmount($array, $st_date, $ed_date)
    {
        ...
        if (!empty($ar)) {
            foreach ($ar as $row_key => $row_value) {
                if (!isset($row_value->date)) {
                    continue;
                }
                if ($this->isDateWithinRange((string) $row_value->date, $st_date, $ed_date, isset($array->created_at) ? $array->created_at : null)) {
                    $array[] = $row_value;
                }
            }
        }
        ...
        if (!empty($ar)) {
            foreach ($ar as $row_key => $row_value) {
                if (isset($row_value->received_by)) {
                    if (isset($row_value->date) && $this->isDateWithinRange((string) $row_value->date, $st_date, $ed_date, isset($array->created_at) ? $array->created_at : null) && $row_value->received_by == $receivedBy) {
                        $array[] = $row_value;
                    }
                }
            }
     ...
     */
    private function normalizeDateString($rawDate)
    {
        ...
        if (preg_match('/^(\d{1,2})[-\/\.](\d{1,2})[-\/\.](\d{2,4})$/', $rawDate, $m)) {
            $year = (int) $m[3];
            if ($year < 100) {
                // Keep existing project convention for 2-digit years.
                $year += 2000;
            }
            return $year . '-' . str_pad($m[1], 2, '0', STR_PAD_LEFT) . '-' . str_pad($m[2], 2, '0', STR_PAD_LEFT);
        }
    ...
    /**
     * Match both AD and BS stored dates against AD range used by reports.
     */
    private function isDateWithinRange($rawDate, $st_date, $ed_date, $createdAt = null)
    {
        ...
        // If this looks like BS (e.g. 2082-..), use created_at fallback (AD timestamp)
        // because this report's search window is AD-based.
        if ($year >= 2070) {
            if (!empty($createdAt)) {
                $created_ts = strtotime($createdAt);
                if ($created_ts !== false) {
                    // Compare by calendar day so end-date is inclusive.
                    $start_day   = strtotime(date('Y-m-d', $st_date));
                    $end_day     = strtotime(date('Y-m-d', $ed_date));
                    $created_day = strtotime(date('Y-m-d', $created_ts));
                    return ($created_day >= $start_day && $created_day <= $end_day);
                }
            }

            // Backward fallback to previous BS string-range behavior.
            $start_bs = $this->customlib->convertADDateToBS(date('Y-m-d', $st_date));
            $end_bs = $this->customlib->convertADDateToBS(date('Y-m-d', $ed_date));
            return ($normalized >= $start_bs && $normalized <= $end_bs);
        }

        $row_ts = strtotime($normalized);
        if ($row_ts === false) {
            return false;
        }
        return ($row_ts >= $st_date && $row_ts <= $ed_date);
    }
    ...
    public function fee_deposit_collections($data)
    {
        if (!empty($data)) {
            ...
            foreach ($data as $d_key => $d_value) {
                ...
                $q = $this->db->get('student_fees_deposite');
                $student_fees_discount_id = isset($d_value['student_fees_discount_id']) ? $d_value['student_fees_discount_id'] : null;
                $fee_groups_feetype_saved = isset($d_value['fee_groups_feetype_id']) ? (int) $d_value['fee_groups_feetype_id'] : 0;
                unset($data[$d_key]['student_fees_discount_id']);
                ...
                if ($q->num_rows() > 0) {
                    ...
                    $payment_ref = $row->id . '//' . $inv_no;
                    if ($student_fees_discount_id != null && !empty($a[$inv_no]['amount_discount'])) {
                        $this->feediscount_model->markDiscountUsedAfterPayment(
                            $student_fees_discount_id,
                            $fee_groups_feetype_saved,
                            $d_value['fee_category'],
                            $payment_ref,
                            $desc
                        );
                    }
                    ...
                    if ($student_fees_discount_id != null && !empty($d_value['amount_detail']['amount_discount'])) {
                        $this->feediscount_model->markDiscountUsedAfterPayment(
                            $student_fees_discount_id,
                            $fee_groups_feetype_saved,
                            $d_value['fee_category'],
