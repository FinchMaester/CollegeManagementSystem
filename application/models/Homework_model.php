...
class Homework_model extends MY_model
{
    ...
    public function dailyassignmentreport($class_id, $section_id, $subject_group_id, $subject_group_subject_id, $condition = null)
    {
        ...
        $this->datatables->select('students.id as student_id, students.firstname, students.middlename, students.lastname, students.admission_no as student_admission_no, classes.class, sections.section, subjects.name as subject_name, COUNT(daily_assignment.id) as total_assignment, MAX(daily_assignment.date) as last_assignment_date, MAX(staff.name) as staff_name, MAX(staff.surname) as staff_surname, MAX(staff.employee_id) as staff_employee_id')
            ->searchable('students.firstname,classes.class,sections.section')
            ->join("student_session", "student_session.id = daily_assignment.student_session_id")
            ->join("classes", "classes.id = student_session.class_id")
            ->join("sections", "sections.id = student_session.section_id")
            ->join("students", "students.id = student_session.student_id")
            ...
            ->join("subjects", "subjects.id = subject_group_subjects.subject_id")
            ->join("staff", "staff.id = `daily_assignment`.`evaluated_by`", "left")
            ->orderable('students.firstname,classes.class,sections.section')
            ->group_by('students.id, students.firstname, students.middlename, students.lastname, students.admission_no, classes.class, sections.section, subjects.name')
            ->sort('students.firstname,classes.class,sections.section', 'DESC')
            ->from('daily_assignment');
