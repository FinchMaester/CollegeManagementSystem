...
class Subjectgroup_model extends MY_Model {
    ...
    public function edit($data, $delete_sections, $add_sections, $delete_subjects, $add_subjects) {
        ...
        log_message('debug', 'Subjectgroup Model: edit() - Data: ' . print_r($data, true));
        log_message('debug', 'Subjectgroup Model: edit() - Sections to Delete: ' . print_r($delete_sections, true));
        log_message('debug', 'Subjectgroup Model: edit() - Sections to Add: ' . print_r($add_sections, true));
        log_message('debug', 'Subjectgroup Model: edit() - Subjects to Delete: ' . print_r($delete_subjects, true));
        log_message('debug', 'Subjectgroup Model: edit() - Subjects to Add: ' . print_r($add_subjects, true));
        ...
        if (!empty($add_sections)) {
            ...
            foreach ($add_sections as $section_group_key => $section_group_value) {
                // Get class_section_id from faculty_id and class_id
                $this->db->select('id');
                $this->db->from('class_sections');
                $this->db->where('section_id', $section_group_value['faculty_id']);
                $this->db->where('class_id', $section_group_value['class_id']);
                $this->db->where('program_id', $section_group_value['program_id']);
                $query = $this->db->get();
                ...
                if ($class_section) {
                    $sections_array = array(
                        ...
                        'program_id' => $section_group_value['program_id'],
                        'session_id' => $this->setting_model->getCurrentSession(),
                    );
                    $section_group_array[] = $sections_array;
                } else {
                    log_message('error', 'Subjectgroup Model: edit() - Could not find class_section for faculty_id: ' . $section_group_value['faculty_id'] . ', program_id: ' . $section_group_value['program_id'] . ', class_id: ' . $section_group_value['class_id']);
                }
            }
            if (!empty($section_group_array)) {
                $this->db->insert_batch('subject_group_class_sections', $section_group_array);
                log_message('debug', 'Subjectgroup Model: edit() - Inserted ' . $this->db->affected_rows() . ' new subject group sections.');
            } else {
                log_message('debug', 'Subjectgroup Model: edit() - No new subject group sections to insert (section_group_array is empty).');
            }
        }
        ...
        if (!empty($delete_sections)) {
            foreach ($delete_sections as $section) {
                // Get class_section_id from faculty_id, program_id, and class_id
                $this->db->select('id');
                $this->db->from('class_sections');
                $this->db->where('section_id', $section['faculty_id']);
                $this->db->where('program_id', $section['program_id']);
                ...
                if ($class_section) {
                    ...
                    $this->db->where('class_section_id', $class_section->id);
                    $this->db->where('program_id', $section['program_id']);
                    $this->db->delete('subject_group_class_sections');
                    log_message('debug', 'Subjectgroup Model: edit() - Deleted subject group section: ' . print_r($section, true) . '. Affected rows: ' . $this->db->affected_rows());
                } else {
                    log_message('error', 'Subjectgroup Model: edit() - Could not find class_section to delete for faculty_id: ' . $section['faculty_id'] . ', program_id: ' . $section['program_id'] . ', class_id: ' . $section['class_id']);
                }
            }
        }
    ...
    public function add($data, $subject_group, $section_group) {
        log_message('debug', 'Subjectgroup Model: add() - Subject Group Data: ' . print_r($data, true));
        log_message('debug', 'Subjectgroup Model: add() - Subjects: ' . print_r($subject_group, true));
        log_message('debug', 'Subjectgroup Model: add() - Sections: ' . print_r($section_group, true));
        ...
        foreach ($section_group as $section_group_key => $section_group_value) {
            ...
            if ($class_section) {
                ...
                log_message('error', 'Subjectgroup Model: add() - Could not find class_section for faculty_id: ' . $section_group_value['faculty_id'] . ', program_id: ' . $section_group_value['program_id'] . ', class_id: ' . $section_group_value['class_id']);
            }
        }
        if (!empty($section_group_array)) {
            $this->db->insert_batch('subject_group_class_sections', $section_group_array);
            log_message('debug', 'Subjectgroup Model: add() - Inserted ' . $this->db->affected_rows() . ' subject group sections.');
        } else {
            log_message('debug', 'Subjectgroup Model: add() - No subject group sections to insert (section_group_array is empty).');
        }
    ...
    public function getClassSectionByGroup($subject_group_id) {
        $sql = "SELECT subject_group_class_sections.*,
                classes.id as `class_id`, classes.class,
                sections.id as `section_id`, sections.section,
                programs.id as program_id, programs.title as program_name
                ...
                LEFT JOIN programs on programs.id=subject_group_class_sections.program_id
    ...
    public function getGroupByClassSectionAndProgram($class_id, $section_id, $program_id) {
        return $this->getGroupsBySectionProgramClass($section_id, $program_id, $class_id);
    }

    /**
     * Subject groups linked to faculty (section), program, and/or academic level (class)
     * for the current session — used by library member search and similar filters.
     *
     * @param int $section_id  Faculty / section id (0 = any)
     * @param int $program_id  Program id (0 = any)
     * @param int $class_id    Academic level / class id (0 = any)
     * @return array
     */
    public function getGroupsBySectionProgramClass($section_id = 0, $program_id = 0, $class_id = 0)
    {
        $section_id = (int) $section_id;
        $program_id = (int) $program_id;
        $class_id   = (int) $class_id;

        $this->db->select('DISTINCT subject_groups.id, subject_groups.name', false);
        $this->db->from('subject_group_class_sections sgcs');
        $this->db->join('subject_groups', 'subject_groups.id = sgcs.subject_group_id');
        $this->db->join('class_sections cs', 'cs.id = sgcs.class_section_id');
        $this->db->where('sgcs.session_id', $this->current_session);
        $this->db->where('subject_groups.session_id', $this->current_session);
        ...
        if ($program_id > 0) {
            $this->db->group_start();
            $this->db->where('sgcs.program_id', $program_id);
            $this->db->or_where('sgcs.program_id IS NULL', null, false);
            $this->db->or_where('sgcs.program_id', 0);
            $this->db->group_end();
        }

        $this->db->order_by('subject_groups.name', 'ASC');
        $query = $this->db->get();
        return $query->result_array();
    }

    /**
     * Get subjects by class, section, and optional date (with timetable info)
     * This method fetches subjects from subject groups assigned to the class/section
     * Date is optional - if provided, filters timetable by day; otherwise shows all subjects
     */
    public function getSubjectsByClassSectionWithTimetable($class_id, $section_id, $date = null) {
        $day = null;
        if ($date && !empty($date)) {
            $day = date('l', strtotime($date));
        }
        
        // Fetch subjects from subject groups assigned to this class/section
        // subject_group_class_sections uses class_section_id, so we need to JOIN with class_sections
        ...
        $sql = "SELECT 
                    ...
                    COALESCE(staff.name, '') as name,
                    COALESCE(staff.surname, '') as surname,
                    COALESCE(staff.employee_id, '') as employee_id
        ...
        if ($day) {
            $sql .= " AND st.day = " . $this->db->escape($day);
        }
        
        $sql .= " LEFT JOIN staff ON staff.id = st.staff_id
                WHERE cs.class_id = " . $this->db->escape($class_id) . "
                    AND cs.section_id = " . $this->db->escape($section_id) . "
                    AND sgcs.session_id = " . $this->db->escape($this->current_session) . "
                    AND sgs.session_id = " . $this->db->escape($this->current_session) . "
                GROUP BY subjects.id, st.id, st.time_from, st.time_to, staff.name, staff.surname, staff.employee_id
                ORDER BY subjects.name ASC, COALESCE(st.time_from, '') ASC";
