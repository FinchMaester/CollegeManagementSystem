...
class Librarymanagement_model extends MY_Model
{
    ...
    public function check_data_exists($data)
    {
        ...
        // If member_id and member_type are provided, only check if card exists for a DIFFERENT member
        // This allows the same member to use their own card number (updating their own card)
        if (isset($data['member_id']) && isset($data['member_type'])) {
            $this->db->group_start();
            $this->db->where('member_id !=', $data['member_id']);
            $this->db->or_where('member_type !=', $data['member_type']);
            $this->db->group_end();
        }
