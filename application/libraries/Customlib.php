...
class Customlib
{
    ...
    /**
     * Absolute filesystem path for a DB-stored upload (e.g. uploads/student_images/...).
     * Tries FCPATH first, then optional admin folder_path root (sch_settings).
     *
     * @param string|null $path
     * @return string|null
     */
    public function resolveStudentUploadDiskPath($path)
    {
        ...
        if (preg_match('#^[a-z]:/#i', $norm)) {
            $win = str_replace('/', DIRECTORY_SEPARATOR, $path);
            return is_file($win) ? $win : $win;
        }
        if (strpos($norm, '/') === 0) {
     ...
     * @return bool
     */
    public function studentUploadFileExists($path)
    {
        $disk = $this->resolveStudentUploadDiskPath($path);
        return $disk !== null && is_file($disk);
    }

    /**
     * Public URL for img/src from students.image / father_pic / etc.
     *
     * @param string|null $path
     * @return string
     */
    public function studentUploadPublicUrl($path)
    {
        $path = trim((string) $path);
        if ($path === '') {
            return '';
    ...
    /**
     * URL for student_doc.doc — filename only or full relative uploads/... path.
     *
     * @param int    $studentId
     * @param string $doc
     * @return string
     */
    public function studentDocumentPublicUrl($studentId, $doc)
    {
        $doc = trim((string) $doc);
        if ($doc === '') {
            return '';
        ...
        if (strpos($doc, 'uploads/') === 0) {
            return rtrim(base_url(), '/') . '/' . $doc;
        }
        $base = rtrim(base_url(), '/') . '/uploads/student_documents/' . (int) $studentId . '/';
        return $base . rawurlencode(basename($doc));
    }
    ...
    public function getSchoolCurrencyFormat()
    {
        $admin = $this->CI->session->userdata('admin');
        if ($admin) {
            $setting = $this->CI->setting_model->get(1);
            $currency_id = 0;
            if (!empty($admin['id'])) {
                $staff_row = $this->CI->db->select('currency_id')->from('staff')->where('id', (int) $admin['id'])->limit(1)->get()->row();
                if (!empty($staff_row) && (int) $staff_row->currency_id > 0) {
                    $currency_id = (int) $staff_row->currency_id;
                }
            }
            if ($currency_id < 1 && !empty($setting['currency_id'])) {
                $currency_id = (int) $setting['currency_id'];
            } elseif ($currency_id < 1 && !empty($setting['currency'])) {
                $currency_id = (int) $setting['currency'];
            } elseif ($currency_id < 1 && !empty($admin['currency'])) {
                $currency_id = (int) $admin['currency'];
            }
            if ($currency_id > 0) {
                $this->CI->load->model('currency_model');
                $currency_row = $this->CI->currency_model->get($currency_id);
                if (!empty($currency_row->symbol)) {
                    return $currency_row->symbol;
                }
            }
            if (!empty($setting['currency_symbol'])) {
                return $setting['currency_symbol'];
            }
            return isset($admin['currency_symbol']) ? $admin['currency_symbol'] : '';
        } else if ($this->CI->session->userdata('student')) {
    ...
    public function getYearDropdown($years_back = 100, $years_forward = 0)
    {
        $array        = array();
        $current_year = (int) date('Y');
        $start_year   = $current_year + (int) $years_forward;
        $end_year     = $current_year - (int) $years_back;
    ...
    public function getNepaliMonthDropdown()
    {
        $nepali_months = array(
            '01' => 'बैशाख',
            '02' => 'जेठ',
    ...
    public function getNepaliYearDropdown($years_back = 10)
    {
        $array = array();
        $current_year = date('Y');
        
        // Convert current year to Nepali year (approximately)
        $nepali_year = $current_year + 57; // Rough conversion
        
        for ($i = 0; $i < $years_back; $i++) {
            $year_val = $nepali_year - $i;
            $array[$year_val] = $year_val . ' (BS)';
        }
    ...
    /**
     * Convert AD session to BS session
     * Example: 2024-25 AD = 2081-82 BS, 2025-26 AD = 2082-83 BS
     * Conversion: AD year + 57 = BS year
     * 
     * @param string $adSession Session in AD format (e.g., "2024-25")
     * @return string Session in BS format (e.g., "2081-82")
     */
    public function convertSessionToBS($adSession)
    {
        if (empty($adSession)) {
            return $adSession;
        }
        
        $parts = explode('-', $adSession);
        if (count($parts) == 2) {
            $year1 = intval($parts[0]);
            $year2 = intval($parts[1]);
            
            // Handle 2-digit years (e.g., "25" in "2024-25")
            if ($year2 < 100) {
                // Extract century from first year
                $century = intval(substr($parts[0], 0, 2));
                $year2 = $century * 100 + $year2;
            }
            
            // Convert: AD year + 57 = BS year
            $bsYear1 = $year1 + 57;
            $bsYear2 = $year2 + 57;
            
            // Extract last 2 digits for second year to maintain format (e.g., "82" instead of "2082")
            $bsYear2Short = substr($bsYear2, -2);
            
            return $bsYear1 . '-' . $bsYear2Short;
        }
    ...
    /**
     * Check if a session year is in valid BS range (2070-2100)
     * @param int $year Year to check
     * @return bool True if in valid BS range
     */
    private function isValidBSYear($year) {
        // Valid BS years are typically 2070-2100 (current is around 2081-2082)
        return ($year >= 2070 && $year <= 2100);
    }
    
    /**
     * Convert session to BS format if it's in AD format, otherwise return as-is
     * @param string $session Session string (e.g., "2024-25" or "2081-82")
     * @return string BS formatted session
     */
    public function convertSessionToBSIfNeeded($session)
    {
        if (empty($session)) {
            return $session;
        }
        ...
        if (count($parts) == 2) {
            ...
            // This handles:
            // - Years < 2070 (e.g., 2024-25 AD -> 2081-82 BS)
            // - Years > 2100 (e.g., 2138-39 incorrectly stored -> convert to BS)
            return $this->convertSessionToBS($session);
        }
    ...
    /**
     * Convert BS session to AD session.
     * Example: 2082-83 BS = 2025-26 AD (BS year - 57 = AD year)
     *
     * @param string $bsSession
     * @return string
     */
    public function convertSessionToAD($bsSession)
    {
        if (empty($bsSession)) {
            return $bsSession;
        }

        $parts = explode('-', (string) $bsSession);
        if (count($parts) != 2) {
            return $bsSession;
        }
        ...
        $year2 = (int) $parts[1];
        if ($year2 < 100) {
            $century = (int) substr((string) $parts[0], 0, 2);
            $year2 = ($century * 100) + $year2;
        }

        $adYear1 = $year1 - 57;
        $adYear2 = $year2 - 57;
        $adYear2Short = substr((string) $adYear2, -2);

        return $adYear1 . '-' . $adYear2Short;
    }

    /**
     * Ensure session is in AD format (convert from BS when needed).
     *
     * @param string $session
     * @return string
     */
    public function convertSessionToADIfNeeded($session)
    {
        if (empty($session)) {
            return $session;
        }
        ...
        $year1 = (int) $parts[0];
        // Already AD-like
        if ($year1 > 0 && $year1 < 2070) {
            $year2 = (int) $parts[1];
            if ($year2 < 100) {
                return $year1 . '-' . str_pad((string) $year2, 2, '0', STR_PAD_LEFT);
            }
            return $year1 . '-' . substr((string) $year2, -2);
        }

        return $this->convertSessionToAD($session);
    }

    /**
     * Convert AD date (YYYY-MM-DD) to BS date (YYYY-MM-DD)
     * Simple conversion: AD Year + 57 = BS Year
     * Note: This is approximate - for exact conversion, use a proper Nepali calendar library
     * 
     * @param string $adDate Date in AD format (e.g., "2025-01-01")
     * @return string Date in BS format (e.g., "2082-01-01")
     */
    public function convertADDateToBS($adDate)
    {
        ...
        if (count($parts) == 3) {
            ...
            // Convert year: AD + 57 = BS
            $bsYear = $adYear + 57;
            
            return sprintf('%04d-%02s-%02s', $bsYear, $month, $day);
        }
    ...
    /**
     * @return \Nilambar\NepaliDate\NepaliDate|null
     */
    private function nepaliDateLib()
    {
        static $lib = null;
        ...
        if ($lib === null) {
            if (!class_exists('\Nilambar\NepaliDate\NepaliDate')) {
                $missing = true;
                return null;
            }
            $lib = new \Nilambar\NepaliDate\NepaliDate();
        }
        return $lib;
    }

    /**
     * Start of "today" in Asia/Kathmandu as AD Unix timestamp (library due vs real calendar).
     *
     * @return int|false
     */
    public function libraryTodayAdStartTimestamp()
    {
        try {
    ...
    /**
     * Parse a leading Y-M-D (or Y/M/D) from library VARCHAR dates; pads to Y-m-d.
     * Accepts unpadded months/days as stored by the Nepali datepicker (e.g. 2082-5-10).
     * BS months may have up to 32 days.
     *
     * @param string|null $date
     * @return string Normalized Y-m-d or '' if empty / invalid / 0000-00-00
     */
    public function parseLibraryDateYmd($date)
    {
        if ($date === null || $date === '') {
            return '';
        }
        $s = trim((string) $date);
        if ($s === '' || substr($s, 0, 10) === '0000-00-00') {
            return '';
        }
    ...
    /**
     * True if a due date (BS or legacy AD VARCHAR) is before today in the real Bikram Sambat calendar.
     * Uses ernilambar/nepali-date when available; falls back to year−57 approximation if the package is missing.
     *
     * @param string|null $due_raw
     * @return bool
     */
    public function isLibraryDueDateOverdue($due_raw)
    {
        $ymd = $this->parseLibraryDateYmd($due_raw);
        if ($ymd === '') {
            return false;
        }
        ...
        $today_ts = $this->libraryTodayAdStartTimestamp();
        if ($today_ts === false) {
            return false;
        }
        $due_ts = false;
        if ($y >= 2070 && $y <= 2100) {
            $nepali = $this->nepaliDateLib();
            if ($nepali !== null) {
                $ad = $nepali->convertBsToAd($y, $m, $d);
                if (!empty($ad) && isset($ad['year'], $ad['month'], $ad['day'])) {
                    $due_ts = strtotime(sprintf('%04d-%02d-%02d', (int) $ad['year'], (int) $ad['month'], (int) $ad['day']));
                }
            }
            if ($due_ts === false || $due_ts < 0) {
                $ad_year = $y - 57;
                $due_ts = strtotime(sprintf('%04d-%02d-%02d', $ad_year, $m, $d));
            }
        } else {
            $due_ts = strtotime($ymd);
        }
        if ($due_ts === false) {
            return false;
        }
        return (int) $due_ts < (int) $today_ts;
    }

    /**
     * Convert a library VARCHAR date (BS or AD) to AD midnight timestamp.
     *
     * @param string|null $date_raw
     * @return int|false
     */
    public function libraryDateToAdTimestamp($date_raw)
    {
        $ymd = $this->parseLibraryDateYmd($date_raw);
        if ($ymd === '') {
            return false;
        }
        $y = (int) substr($ymd, 0, 4);
        ...
        if ($y >= 2070 && $y <= 2100) {
            ...
            if ($nepali !== null) {
                $ad = $nepali->convertBsToAd($y, $m, $d);
                if (!empty($ad) && isset($ad['year'], $ad['month'], $ad['day'])) {
                    return strtotime(sprintf('%04d-%02d-%02d', (int) $ad['year'], (int) $ad['month'], (int) $ad['day']));
                }
            }
            return strtotime(sprintf('%04d-%02d-%02d', $y - 57, $m, $d));
        }
        if ($y >= 1900 && $y < 2070) {
    ...
    /**
     * @param int $timestamp AD midnight
     * @return string BS Y-m-d
     */
    public function libraryAdTimestampToBsYmd($timestamp)
    {
        if ($timestamp === false || $timestamp < 1) {
            return '';
        }
        $adY = (int) date('Y', $timestamp);
        $adM = (int) date('n', $timestamp);
        $adD = (int) date('j', $timestamp);
        $nepali = $this->nepaliDateLib();
        if ($nepali !== null) {
            $bs = $nepali->convertAdToBs($adY, $adM, $adD);
            if (!empty($bs) && isset($bs['year'], $bs['month'], $bs['day'])) {
                return sprintf('%04d-%02d-%02d', (int) $bs['year'], (int) $bs['month'], (int) $bs['day']);
            }
        }
        return $this->convertADDateToBS(sprintf('%04d-%02d-%02d', $adY, $adM, $adD));
    }

    /**
     * Add calendar days to a library date; returns BS Y-m-d when input is BS.
     *
     * @param string|null $date_raw
     * @param int         $days
     * @return string
     */
    public function libraryAddDays($date_raw, $days)
    {
        $days = (int) $days;
        if ($days < 1) {
            return $this->parseLibraryDateYmd($date_raw);
        }
        $ts = $this->libraryDateToAdTimestamp($date_raw);
        if ($ts === false) {
            return '';
        }
        ...
        return $this->libraryAdTimestampToBsYmd($new_ts);
    }

    /**
     * Whole days a book is overdue (0 if not overdue).
     *
     * @param string|null $due_raw
     * @param string|null $as_of_raw optional library date; defaults to today
     * @return int
     */
    public function libraryOverdueDays($due_raw, $as_of_raw = null)
    {
        $due_ts = $this->libraryDateToAdTimestamp($due_raw);
        if ($due_ts === false) {
            return 0;
        }
        if ($as_of_raw !== null && $as_of_raw !== '') {
            $as_ts = $this->libraryDateToAdTimestamp($as_of_raw);
        } else {
            $as_ts = $this->libraryTodayAdStartTimestamp();
        }
        if ($as_ts === false) {
            return 0;
        }
        $diff = (int) floor(((int) $as_ts - (int) $due_ts) / 86400);
        return $diff > 0 ? $diff : 0;
    }

    /**
     * Format a library book_issues date stored as VARCHAR (Nepali picker: Y-m-d BS).
     * Returns Y-m-d in BS for display. Legacy AD rows (typical Gregorian years) are converted with convertADDateToBS.
     *
     * @param string|null $date Raw value from DB
     * @return string Normalized BS Y-m-d, or empty string if invalid / empty
     */
    public function formatLibraryDateBsForDisplay($date)
    {
        $ymd = $this->parseLibraryDateYmd($date);
        if ($ymd === '') {
            $s = trim((string) $date);
            if ($s === '' || substr($s, 0, 10) === '0000-00-00') {
                return '';
        ...
        if ($y >= 1900 && $y < 2070) {
            return $this->convertADDateToBS($ymd);
        }
        return '';
    }
    ...
    public function datetostrtotime($date)
    {
        ...
        if ($ymd !== '') {
            ...
            if ($year >= 2070 && $year <= 2100) {
                $nepali = $this->nepaliDateLib();
                if ($nepali !== null) {
                    $ad = $nepali->convertBsToAd($year, $month, $day);
                    if (!empty($ad) && isset($ad['year'], $ad['month'], $ad['day'])) {
                        return strtotime(sprintf('%04d-%02d-%02d', (int) $ad['year'], (int) $ad['month'], (int) $ad['day']));
                    }
                }
                $ad_year = $year - 57;
                $ad_ymd = sprintf('%04d-%02d-%02d', $ad_year, $month, $day);
                return strtotime($ad_ymd);
            }
            return strtotime($ymd);
        }
        ...
        // If parsing failed, try strtotime directly
    ...
    public function dateFormatToYYYYMMDD($date = null)
    {
        ...
        // Handle Nepali datepicker format (2082-03-20, 2082/03/20, 2082.3.20, 2082-3-20, 3-20-82, etc)
        $date = trim((string) $date);
        if ($date === '') {
            return null;
        }
        // If already in YYYY-MM-DD or similar, just pad
        ...
        // If in M-D-YY or M/D/YY or M.D.YY
        if (preg_match('/^(\d{1,2})[-\/\.](\d{1,2})[-\/\.](\d{2})$/', $date, $m)) {
            $year = (intval($m[3]) > 80) ? (1900 + intval($m[3])) : (2000 + intval($m[3]));
            $month = str_pad($m[1], 2, '0', STR_PAD_LEFT);
            $day = str_pad($m[2], 2, '0', STR_PAD_LEFT);
            return "$year-$month-$day";
        }
        // Fallback to school date format (English / AD calendar display strings).
        $format = $this->getSchoolDateFormat();
        $date_formated = date_parse_from_format($format, $date);
        $year          = (int) $date_formated['year'];
        $month         = (int) $date_formated['month'];
        $day           = (int) $date_formated['day'];
    ...
    /**
     * Get date ranges in Nepali (BS) calendar format for income search
     * This function converts English calendar date ranges to Nepali calendar date ranges
     * Since income dates are stored in BS format in the database
     */
    public function get_betweendate_bs($search_type)
    {
        $ad_dates = $this->get_betweendate($search_type);

        if (empty($ad_dates['from_date']) || empty($ad_dates['to_date'])) {
            return $ad_dates;
        }

        $from_ts = strtotime($ad_dates['from_date']);
        $to_ts   = strtotime($ad_dates['to_date']);
        if ($from_ts === false || $to_ts === false) {
            return array(
                'from_date' => $this->parseLibraryDateYmd($ad_dates['from_date']),
                'to_date'   => $this->parseLibraryDateYmd($ad_dates['to_date']),
            );
        }

        return array(
            'from_date' => $this->libraryAdTimestampToBsYmd($from_ts),
            'to_date'   => $this->libraryAdTimestampToBsYmd($to_ts),
        );
    }
    ...
    /**
     * Filesystem-safe barcode file name. admission_no / employee_id can contain '/', spaces, etc.
     * which would otherwise create non-existent nested folders and make imagepng() fail.
     * Only the file name is sanitized; the encoded barcode text still uses the original value.
     *
     * @param string $code
     * @return string
     */
    protected function barcode_safe_filename($code)
    {
        $safe = preg_replace('/[^A-Za-z0-9._-]+/', '_', (string) $code);
        $safe = trim($safe, '_');
        return $safe !== '' ? $safe : 'barcode';
    }

    /**
     * Relative path to student barcode PNG (uploads/student_id_card/barcodes/{admission_no}.png).
     *
     * @param string $admission_no
     * @return string empty on failure
     */
    public function student_barcode_relative_path($admission_no)
    {
        $code = trim((string) $admission_no);
        if ($code === '') {
            return '';
        }
        return 'uploads/student_id_card/barcodes/' . $this->barcode_safe_filename($code) . '.png';
    }

    /**
     * Return existing student barcode path or generate a Code128 PNG (admission number).
     *
     * @param string $admission_no
     * @return string web-relative path or empty string
     */
    public function ensure_student_barcode($admission_no)
    {
        $rel = $this->student_barcode_relative_path($admission_no);
        if ($rel === '') {
            return '';
        }
        ...
        try {
            return $this->generatebarcode($admission_no);
        } catch (Exception $e) {
            log_message('error', 'ensure_student_barcode failed for ' . $admission_no . ': ' . $e->getMessage());
            return '';
        }
    }
    ...
    public function generatebarcode($admission_no)
    {
        ...
        $rel = $this->student_barcode_relative_path($code);
        $dir = FCPATH . 'uploads' . DIRECTORY_SEPARATOR . 'student_id_card' . DIRECTORY_SEPARATOR . 'barcodes';
        if (!is_dir($dir)) {
            @mkdir($dir, 0777, true);
        }
        $abs = FCPATH . str_replace('/', DIRECTORY_SEPARATOR, $rel);
    ...
    /**
     * Return existing staff barcode path or generate Code128 PNG.
     *
     * @param string $code employee_id, library_card_no, etc.
     * @return string
     */
    public function ensure_staff_barcode($code)
    {
        $code = trim((string) $code);
        if ($code === '') {
            return '';
        }
        $rel = 'uploads/staff_id_card/barcodes/' . $this->barcode_safe_filename($code) . '.png';
        $abs = FCPATH . str_replace('/', DIRECTORY_SEPARATOR, $rel);
        if (is_readable($abs)) {
            return $rel;
        }
        try {
            return $this->generatestaffbarcode($code);
        } catch (Exception $e) {
            log_message('error', 'ensure_staff_barcode failed for ' . $code . ': ' . $e->getMessage());
            return '';
        }
    }
    ...
    {
        $code = trim((string) $employee_id);
        if ($code === '') {
            return '';
        }
        $rel = 'uploads/staff_id_card/barcodes/' . $this->barcode_safe_filename($code) . '.png';
        $dir = FCPATH . 'uploads' . DIRECTORY_SEPARATOR . 'staff_id_card' . DIRECTORY_SEPARATOR . 'barcodes';
        if (!is_dir($dir)) {
            @mkdir($dir, 0777, true);
        }
        $abs = FCPATH . str_replace('/', DIRECTORY_SEPARATOR, $rel);
