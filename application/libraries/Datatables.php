...
class Datatables
{
    ...
    public function orderable($columns, $backtick_protect = true)
    {
        foreach ($this->explode(',', $columns) as $val) {
            $column = trim($val);
            // Only strip "... AS alias" for plain columns. Leave SQL expressions alone
            // (e.g. CAST(x AS UNSIGNED) must not become just "UNSIGNED").
    ...
    private function get_ordering()
    {
        ...
        if ($this->ci->input->post('order')) {
            ...
            foreach ($this->ci->input->post('order') as $key) {
                $col = isset($this->orderable[$key['column']]) ? $this->orderable[$key['column']] : '';
                if ($col === '') {
                    continue;
                }
                // Do not backtick-protect SQL expressions used for natural/numeric sorts.
                $escape = (strpos($col, '(') === false && strpos($col, '+') === false);
                $this->ci->db->order_by($col, $key['dir'], $escape);
            }
        }
        ...
        foreach ($this->sortable as $val) {
            $col = $val[0];
            $escape = (strpos($col, '(') === false && strpos($col, '+') === false);
            $this->ci->db->order_by($col, $val[1], $escape);
        }
    }
