    ...
    public function mailsms($send_for, $sender_details, $date = null, $exam_schedule_array = null, $file = null)
    {
        ...
        if (!empty($chk_mail_sms)) {
            ...
            if ($chk_mail_sms['sms'] && $chk_mail_sms['template'] != "" && !empty($sms_detail)) {
                ...
                // Send to the submitted email: admin uses staff_email, user/guardian uses email or guardian_email.
                // Do not require staff_recipient so password reset works even if that notification flag is off.
                // Return send result so controller can show error when mail fails.
                $to_email = null;
                if (!empty($sender_details['staff_email'])) {
                    $to_email = $sender_details['staff_email'];
                } elseif (!empty($sender_details['email'])) {
                    $to_email = $sender_details['email'];
                } elseif (!empty($sender_details['guardian_email'])) {
                    $to_email = $sender_details['guardian_email'];
                }
                if ($to_email && $chk_mail_sms['mail'] && $chk_mail_sms['template'] != "") {
                    $subject = isset($chk_mail_sms['subject']) ? $chk_mail_sms['subject'] : 'Reset Password';
                    $sent   = $this->CI->mailer->send_mail($to_email, $subject, $msg);
                    return $sent;
                }
                return false;
